...
#define UNC_BG                                                                                                         \
    (10.0 * D2R / 3600.0)    /* default initial gyro bias uncertainty                                                  \
...
static void getGn(const insopt_t *opt, const insstate_t *ins, const double dt, double *Gn)
{
...
static void getprn(const insstate_t *ins, const insopt_t *opt, double dt, double *Q)
{
    ...
    for (i = INBA; i < INBA + NNBA(opt); i++)
        Qn[i + i * nprn] = opt->psd.ba * fabs(dt);
    for (i = INBG(opt); i < INBG(opt) + NNBG(opt); i++)
...
extern void initlc(insopt_t *opt, insstate_t *ins)
{
    ...
    ins->xa = mat(ins->nx, 1);
    ins->Pa = mat(ins->nx, ins->nx);
    ins->xb = mat(ins->nb, 1);
    ins->Pb = mat(ins->nb, ins->nb);
    ins->F = eye(ins->nx);
...
static void getphi_euler(const insopt_t *opt, double dt, const double *Cbe, const double *pos, const double *omgb,
                         const double *fib, double *phi)
{
    ...
    {
        for (j = IA; j < IA + NA; j++)
            phi[i + j * nx] -= T2[i - IA + (j - IA) * 3] * dt;
        for (j = ibg; j < ibg + nbg; j++)
            phi[i + j * nx] = T1[i - IA + (j - ibg) * 3] * dt;
        for (j = isg; j < isg + nsg; j++)
            phi[i + j * nx] = T4[i - IA + (j - isg) * 3] * omgb[j - isg] * dt;
        for (j = irg; j < irg + nrg; j++)
            phi[i + j * nx] = T3[i - IA + (j - irg) * 3] * dt;
    }
...
static void getF(const insopt_t *opt, const double *Cbe, const double *pos, const double *omgb, const double *fib,
                 double *F)
{
    int i, j, nx = xnX(opt);
    ...
    for (i = IA; i < IA + NA; i++)
    {
        ...
        for (j = isg; j < isg + nsg; j++)
            F[i + j * nx] = Cbe[i - IA + (j - isg) * 3] * omgb[j - isg];
        for (j = irg; j < irg + nrg; j++)
            F[i + j * nx] = WC[i - IA + (j - irg) * 3];
    }
    ...
    for (i = IV; i < IV + NV; i++)
    {
        ...
        for (j = isa; j < isa + nsa; j++)
            F[i + j * nx] = Cbe[i - IV + (j - isa) * 3] * fib[j - isa];
        for (j = ira; j < ira + nra; j++)
            F[i + j * nx] = WC[i - IV + (j - ira) * 3];
    }
...
static void propP(const insopt_t *opt, const double *Q, const double *phi, const double *P0, double *P)
{
    int i, j, nx = xnX(opt);
...
static void jacobian_p_dt(const double *omgb, const double *lever, const double *Cbe, const double *ve, double *dpddt)
{
    int i;
    double wl[9], cl[9];
    skewsym3(omgb, wl);
...
static void jacobian_v_att(const double *Cbe, const double *lever, const double *omgb, double *dvdatt)
{
    int i;
...
static void jacobian_v_dt(const double *omgb, const double *Cbe, const double *lever, const double *ae, double *dvddt)
{
    int i;
    double wl[9], cl[9];
    skewsym3(omgb, wl);
...
static void jacobian_v_dsg(const double *Cbe, const double *lever, const double *omgb, double *dvds)
{
...
static void jacobian_v_drg(const double *Cbe, const double *lever, const double *omgb, double *dvdrg)
{
    /* dwib=bg+diag(wib)*sg+T_g*r_g (ref[4]) */
 ...
 * -------------------------------------------------------------------------*/
static int build_HVR(const insopt_t *opt, const double *pos, const double *Cbe, const double *lever, const double *omgb,
                     const double *fib, const double *meas, const double *re, const double *ve, const double *ae,
                     const double *std, const double *cov, const double *P, double *H, double *v, double *R)
{
    ...
    for (i = IMP; i < IMP + NMP; i++)
    {
        if (meas[i] != 0.0)
        {
            ...
            {
                for (j = IA; j < IA + NA; j++)
                    H[j + nm * nx] = r1[i - IMP + (j - IA) * 3];
                for (j = IP; j < IP + NP; j++)
                    H[j + nm * nx] = -I[i - IMP + (j - IP) * 3];
                for (j = idt; j < idt + ndt; j++)
                    H[j + nm * nx] = dt1[i - IMP];
                for (j = ila; j < ila + nla; j++)
                    H[j + nm * nx] = -Cbe[i - IMP + (j - ila) * 3];
            }
    ...
    for (i = IMV; i < IMV + NMV; i++)
    {
        if (meas[i] != 0.0)
        {
            ...
            {
                for (j = IA; j < IA + NA; j++)
                    H[j + nm * nx] = v1[i - IMV + (j - IA) * 3];
                for (j = IV; j < IV + NV; j++)
                    H[j + nm * nx] = -I[i - IMV + (j - IV) * 3];
                for (j = ibg; j < ibg + nbg; j++)
                    H[j + nm * nx] = v5[i - IMV + (j - ibg) * 3];
                for (j = idt; j < idt + ndt; j++)
                    H[j + nm * nx] = dt2[i - IMV];
                for (j = isg; j < isg + nsg; j++)
                    H[j + nm * nx] = ds[i - IMV + (j - isg) * 3];
                for (j = irg; j < irg + nrg; j++)
                    H[j + nm * nx] = drg[i - IMV + (j - irg) * 3];
                for (j = ila; j < ila + nla; j++)
                    H[j + nm * nx] = dla[i - IMV + (j - ila) * 3];
            }
...
static int valsol(double *x, double *P, const double *R, const double *v, int nv, double thres)
{
    int i = 0, j;
    ...
    if (x[0] == DISFLAG && norm(x + 0, 3) > 15.0 * D2R)
        i |= 1;
    if (nba && x[iba] == DISFLAG && norm(x + iba, 3) > 1E5 * Mg2M)
        i |= 1;
    if (nbg && x[ibg] == DISFLAG && norm(x + ibg, 3) > 15.0 * D2R)
        i |= 1;
    if (i)
    {
...
/* updates ins states--------------------------------------------------------*/
static void updinss(insstate_t *ins, const double *re, const double *ve, const double *Cbe, const double *fib,
                    const double *omgb, const double *ba, const double *bg, const double *Mgc, const double *Mac,
                    const double *leverc)
{
    trace(3, "updinss:\n");

    matcpy(ins->re, re, 1, 3);
    matcpy(ins->ve, ve, 1, 3);
    matcpy(ins->ba, ba, 1, 3);
    matcpy(ins->bg, bg, 1, 3);
 ...
 * is different
 * --------------------------------------------------------------------------*/
static void interppv(const insstate_t *ins, double dt, double *pe, double *ve, double *ae, double *Cbe)
{
    quat_t q1, q2, q;
    int i;
    ...
    if (fabs(tt) < 1E-6)
    {
        matcpy(pe, ins->re, 1, 3);
        matcpy(ve, ins->ve, 1, 3);
        matcpy(ae, ins->ae, 1, 3);
...
static void prepara(const insstate_t *ins, double *fib, double *omgb, double *Mg, double *Ma, double *Gg, double *ba,
                    double *bg, double *lever)
{
 ...
 * return :none
 * --------------------------------------------------------------------------*/
extern void getaccl(const double *fib, const double *Cbe, const double *re, const double *ve, double *ae)
{
    int i;
    double fe[3], ge[3], cori[3];
...
extern void lcclp(double *x, double *Cbe, double *re, double *ve, double *fib, double *omgb, double *Gg, double *rec,
                  double *vec, double *aec, double *bac, double *bgc, double *Mac, double *Mgc, double *leverc,
                  double *Cbec, double *fibc, double *omgbc, const insopt_t *opt)
{
    ...
    if (nsg && x[isg] != DISFLAG)
    {
        for (i = isg; i < isg + nsg; i++)
            Mgc[i - isg + (i - isg) * 3] += x[i];
    }
    if (nsa && x[isa] != DISFLAG)
    {
        for (i = isa; i < isa + nsa; i++)
            Mac[i - isa + (i - isa) * 3] += x[i];
    }
 ...
 * --------------------------------------------------------------------------*/
static int lcfilt(const insopt_t *opt, insstate_t *ins, const double *meas, const double *std, const double *cov,
                  const double dt, double *x, double *P)
{
    ...
    double rec[3], vec[3], aec[3], Cbec[9], fibc[3], omgbc[3], bac[3], bgc[3], Mac[9], Mgc[9], Gg[9], leverc[3];
...
static void precPhi(const insopt_t *opt, double dt, const double *Cbe, const double *pos, const double *omgb,
                    const double *fib, double *Phi)
{
    int i, nx = xnX(opt);
...
static void updstat(const insopt_t *opt, insstate_t *ins, const double dt, const double *x0, const double *P0,
 ...
 * return : none
 * --------------------------------------------------------------------------*/
extern void propinss(insstate_t *ins, const insopt_t *opt, double dt, double *x, double *P)
{
    int nx = ins->nx;
    double *phi, *Q;
...
extern int savegmeas(insstate_t *ins, const sol_t *sol, const gmea_t *gmea)
{
    ...
    if (sol)
    {
        ...
        for (i = 0; i < 3; i++)
            meas.std[i] = SQRT(sol->qr[i]);
        for (i = 3; i < 6; i++)
            meas.std[i] = SQRT(sol->qv[i - 3]);
...
extern int rechkatt(insstate_t *ins, const imud_t *imu)
{
    ...
    for (i = 0; i < NPOS; i++)
    {
        if (ins->gmeas.data[i].stat <= SOLQ_DGPS)
            return 0;
    }
    ...
    if (ins->gmeas.n == NPOS)
    {
        ...
        for (i = NPOS; i >= 2; i--)
        {
            if ((dt = timediff(ins->gmeas.data[i - 1].t, ins->gmeas.data[i - 2].t)) > 3.0 || fabs(dt) <= 1E-5)
            {
                continue;
            }
            for (j = 0; j < 3; j++)
        ...
        if (norm(vel, 3) > MAXVEL && norm(imu->gyro, 3) < MAXGYRO)
        {
            ...
            if (fabs(yaw - NORMANG(rpy[2] * R2D)) < MAXANG)
                return 0;
#if 0
...
{
    if (data->stat == SOLQ_NONE)
        return 0;
    if (norm(data->pe, 3) == 0.0 && norm(data->ve, 3) == 0.0)
        return 0;
    if (norm(data->std, 6) == 0.0)
        return 0;
    if (data->t.time == 0)
        return 0;
    return 1;
 ...
 * --------------------------------------------------------------------------*/
static int rebootlc(const insopt_t *opt, const gmea_t *data, const imud_t *imu, insstate_t *ins)
{
    ...
    if (!ant2inins(sols[MAXSOLS - 1].t, sols[MAXSOLS - 1].pe, v, opt, NULL, ins, NULL))
    {
 ...
 * --------------------------------------------------------------------------*/
extern int lcigpos(const insopt_t *opt, const imud_t *data, insstate_t *ins, gmea_t *gnss, int upd)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (isnan(ins->re[i]) || isnan(ins->ve[i]) || isnan(ins->ae[i]) || isinf(ins->re[i]) || isinf(ins->ve[i]) ||
            isinf(ins->ae[i]))
        {
 ...
 * return  : 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int ant2inins(gtime_t time, const double *rr, const double *vr, const insopt_t *opt, const imu_t *imu,
                     insstate_t *ins, int *iimu)
{
    ...
    gapv2ipv(rr, vr, ins->Cbe, opt->lever, &imus, ins->re, ins->ve);
    return 1;
}