...
extern int detstc(const imud_t *imu, int n, const insopt_t *opt, const double *pos)
{
    ...
    case IMUDETST_MV:
        return detstatic_MV(imu, n, opt);
    case IMUDETST_MAG:
        return detstatic_MAG(imu, n, opt, pos);
    case IMUDETST_ARE:
        return detstatic_ARE(imu, n, opt);
    case IMUDETST_ALL:
        return detstatic_GLRT(imu, n, opt, pos) && detstatic_MV(imu, n, opt) && detstatic_MAG(imu, n, opt, pos) &&
               detstatic_ARE(imu, n, opt);
    default:
        return detstatic_GLRT(imu, n, opt, pos);
    }
}
