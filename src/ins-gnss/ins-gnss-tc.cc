 ...
 * --------------------------------------------------------------------------*/
static int rebootc(insstate_t *ins, const prcopt_t *opt, const obsd_t *obs, int n, const imud_t *imu, const nav_t *nav)
{
    int i;
    double vr[3] = {0};
    ...
    if (obs == NULL || n <= 0 || chkvb(ins))
        return 0;

    /* initial rtk position when first time  */
    if (first)
    {
        initrtkpos(&rtk, &prcopt);
        first = 0;
    }
    ...
    if (!ant2inins(sols[MAXSOLR - 1].time, sols[MAXSOLR - 1].rr, vr, &opt->insopt, NULL, ins, NULL))
    {
        trace(2, "reboot ins state fail\n");
        return 1;
    }
 ...
 * --------------------------------------------------------------------------*/
extern int tcigpos(const prcopt_t *opt, const obsd_t *obs, int n, const nav_t *nav, const imud_t *imu, rtk_t *rtk,
