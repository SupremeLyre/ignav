 ...
 * return  : none
 * --------------------------------------------------------------------------*/
extern void vr2vb(const double *vr, const double *lever, const double *Cbe, const imud_t *imu, const double s,
                  double *vb)
{
    double T[3], S[9];
 ...
 * return  : none
 * --------------------------------------------------------------------------*/
extern int ve2vr(const double *ve, const double *Cbe, const double *Cbr, const double *lever, const imud_t *imu,
...
static void jacobian_ov_bg(const double *Cbe, const double *Cbr, const double *lever, double *dvdbg)
{
    double sl[9];
    skewsym3(lever, sl);
...
static void jacobian_ov_da(const double *ve, const double *Cbe, const double *Cbr, const double *lever, double *dvdda)
{
    double T[9], sl[9];
 ...
 * return  : none
 * --------------------------------------------------------------------------*/
static void jacobian_ov_dma(const double *vb, const double *Cbe, const double *Cbr, double *dvdma)
{
    double sl[3];
...
static void jacobian_ov_lever(const double *Cbe, const double *Cbr, const double *gyro, double *dvdl)
{
    double T[3], sl[9];
...
static int odoHVR(const insopt_t *opt, const double *Cbe, const double *lever, const double *Cbr, const double *ve,
                  const double s, const imud_t *imu, const odod_t *odo, int nx, double *v, double *H, double *R)
{
    ...
    for (i = 0; i < (opt->odopt.all ? 3 : 1); i++)
    {
        ...
        if (H)
        {
            for (j = ios; j < ios + nos; j++)
                H[j + nv * nx] = ds[i];
            for (j = ioa; j < ioa + noa; j++)
                H[j + nv * nx] = dma[i + (j - ioa) * 3];
            for (j = iol; j < iol + nol; j++)
                H[j + nv * nx] = dl[i + (j - iol) * 3];
            for (j = ibg; j < ibg + nbg; j++)
                H[j + nv * nx] = dbg[i + (j - ibg) * 3];
            for (j = IA; j < IA + NA; j++)
                H[j + nv * nx] = da[i + (j - IA) * 3];
            for (j = IV; j < IV + NV; j++)
                H[j + nv * nx] = ddv[i + (j - IV) * 3];
        }
        r[nv] = VARVEL;
        nv++;
    }
...
static void odoclp(const double *x, const insopt_t *opt, insstate_t *ins)
{
    ...
    for (i = iol; i < iol + nol; i++)
        ins->rbl[i - iol] += x[i];
    clp(ins, opt, x);
}
/* odometry velocity measurement update ins states---------------------------*/
static int odofilt(const insopt_t *opt, const imud_t *imu, const odod_t *odo, insstate_t *ins)
{
    ...
    if ((nv = odoHVR(opt, ins->Cbe, ins->rbl, ins->Cbr, ins->ve, ins->os, imu, odo, nx, v, H, R)))
    {
...
static int odomeas(const insopt_t *opt, const odod_t *odo, odod_t *odom)
{
    ...
    if (dt > (opt->odopt.odt == 0.0 ? 0.5 : opt->odopt.odt))
    {
        odom->dt = dt;
        odom->dr = dr;
        odom->vr[0] = dr / dt;
 ...
 * --------------------------------------------------------------------------*/
extern int odo(const insopt_t *opt, const imud_t *imu, const odod_t *odo, insstate_t *ins)
{
    odod_t odom = {0};
