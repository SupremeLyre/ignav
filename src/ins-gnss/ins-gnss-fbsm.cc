...
typedef struct
{                        /* forward ins solution data type */
...
static int inputimu(const imu_t *imu, imud_t *imudata, const prcopt_t *opt, imud_t *imuz, int ws, int type)
{
    int i, k;
    ...
    { /* backward */
        for (k = ws - 1, i = (iimu + ws >= imu->n ? imu->n - 1 : iimu + ws); k >= 0 && i >= 0 && i < imu->n; k--, i--)
        {
            imuz[k] = imu->data[i];
        }
        *imudata = imu->data[iimu--];
...
static int inputpos(const gsof_data_t *pos, gsof_t *gsofdata, gtime_t timu, const prcopt_t *opt, int type)
{
    int flag = 0, i;
...
{
    if (sols->solq == SOLQ_NONE)
        return 0;
    if (sols->t.time == 0)
        return 0;
    if (norm(sols->pos, 3) == 0.0)
        return 0;
    return 1;
}
...
/* initial ins states--------------------------------------------------------*/
static int init_ins(const imud_t *imu, const gsof_t *pos, const insopt_t *iopt, insstate_t *ins)
{
    ...
    for (i = 0; i < NP; i++)
    {
        if (sols[i].solq >= iopt->iisu || sols[i].solq == SOLQ_NONE)
            return 0;
    }
    ...
    /* initial ins states */
    if (!ant2inins(sols[NP - 1].t, sols[NP - 1].pos, vr, iopt, NULL, ins, NULL))
    {
...
static int open_solfile(const char *file)
{
    ...
    if (!stropen(&frst, STR_FILE, STR_MODE_W, file))
        return 0;
#if !FORWARD_IN_MEMO
    if (!(fp_fwd_sol = fopen(solfile, "wb")))
        return 0;
#endif
    return 1;
}
...
static int search_fsol(int bsize, gtime_t time, ins_fsol_t *sol)
{
    ...
    while (true)
    {
        if (i == 1 && feof(fp_fwd_sol))
            break;
        else if (i == -1 && !ftell(fp_fwd_sol))
            break;

        fseek(fp_fwd_sol, (i == 0 ? -1 : i) * bsize, SEEK_CUR);

        if (!get_time_fsol(fp_fwd_sol, &ts))
            continue;
...
static int bwdinit(const imu_t *imu, const gsof_data_t *pos, rtk_t *rtk)
{
    ...
    for (i = imu->n - 1; i >= 0; i--)
    {
        if (fabs(timediff(rtk->ins.time, imu->data[i].time)) < DTTOL)
            break;
    }
    iimu = i <= 0 ? 0 : i;
...
static int valsmth(const double *Pb, const double *Pf, const double *dx, int nx, const insopt_t *opt)
{
    ...
    /* 1.position */
    for (i = 6; i < 9; i++)
        var[i - 6] = Pb[i + i * nx] + Pf[i + i * nx];
    for (i = 6; i < 9; i++)
    {
        if (SQR(dx[i]) <= SQR(factor) * var[i - 6])
            continue; /* ok if in 4-sigma */
        flag = 3;     /* position error too large */
    }
    /* 2.velocity */
    for (i = 3; i < 6; i++)
        var[i - 3] = Pb[i + i * nx] + Pf[i + i * nx];
    for (i = 3; i < 6; i++)
    {
        if (SQR(dx[i]) <= SQR(factor) * var[i - 3])
            continue;
        flag++;
    }
    /* 3.attitude */
    for (i = 0; i < 3; i++)
        var[i] = Pb[i + i * nx] + Pf[i + i * nx];
    for (i = 0; i < 3; i++)
    {
        if (SQR(dx[i]) <= SQR(factor) * var[i])
            continue;
        flag++;
    }
    /* accl. bias */
    ...
    {
        for (i = 0; i < 3; i++)
            var[i] = Pb[i + xiBa(opt) + (i + xiBa(opt)) * nx] + Pf[i + xiBa(opt) + (i + xiBa(opt)) * nx];
        for (i = 0; i < 3; i++)
        {
            if (SQR(dx[i + xiBa(opt)]) <= SQR(factor) * var[i])
                continue;
            flag++;
        }
    }
    ...
    {
        for (i = 0; i < 3; i++)
            var[i] = Pb[i + xiBg(opt) + (i + xiBg(opt)) * nx] + Pf[i + xiBg(opt) + (i + xiBg(opt)) * nx];
        for (i = 0; i < 3; i++)
        {
            if (SQR(dx[i + xiBg(opt)]) <= SQR(factor) * var[i])
                continue;
            flag++;
        }
    }
...
static int combres(insstate_t *ins, const insopt_t *opt, insstate_t *inss)
{
    ...
    {
        for (i = 0; i < 3; i++)
            dx[xiBa(opt) + i] = ins->ba[i] - fsol.x[xiBa(opt) + i];
    }
    if (xnBg(opt))
    {
        for (i = 0; i < 3; i++)
            dx[xiBg(opt) + i] = ins->bg[i] - fsol.x[xiBg(opt) + i];
    }
    so3_exp(fsol.x, fCbe);
    ...
    for (i = 0; i < 3; i++)
        dx[i] = omg[i];
    for (i = 3; i < 6; i++)
        dx[i] = ins->ve[i - 3] - fsol.x[i];
    for (i = 6; i < 9; i++)
        dx[i] = ins->re[i - 6] - fsol.x[i];
    ...
    if (xnBa(opt))
        matcpy(inss->ba, &fsol.x[xiBa(opt)], 3, 1);
    if (xnBg(opt))
        matcpy(inss->bg, &fsol.x[xiBg(opt)], 3, 1);
    if (xnLa(opt))
        matcpy(inss->lever, &fsol.x[xiLa(opt)], 3, 1);

    lcclp(dxs, fCbe, fsol.x + 6, fsol.x + 3, ins->fb, ins->omgb, ins->Gg, inss->re, inss->ve, inss->ae, inss->ba,
          inss->bg, inss->Ma, inss->Mg, inss->lever, inss->Cbe, inss->fb, inss->omgb, opt);

    matcpy(inss->P, Ps, nx, nx);
    if (ins->stat != INSS_DEGRADE)
...
{
    fp_fwd_sol = fopen(solfile, "rb");
    if (fp_fwd_sol)
        fseek(fp_fwd_sol, 0, SEEK_END);
 ...
 *---------------------------------------------------------------------------*/
static int fbfilt(const imu_t *imu, const gsof_data_t *pos, const prcopt_t *popt, const solopt_t *solopt, rtk_t *rtk,
                  int type)
{
    ...
    while (inputimu(imu, &imus, popt, imuz, ws, type))
    {
        if (inputpos(pos, &poss, imus.time, popt, type))
        {
            ...
            if (!gsof2gnss(&poss, &gmea) || outagegsof(popt, &poss))
                continue;
            if (!init)
            {
                /* initialization */
                if (!(init = init_ins(&imus, &poss, iopt, &rtk->ins)))
                    continue;
            }
            else
            {
...
#if OUT_MONITOR
        /* write solution */
        if (!wrt_solution(type ? &rtks : rtk, solopt, type))
            continue;
#endif
 ...
 * return: number of epochs
 * --------------------------------------------------------------------------*/
extern int lcfbsm(const imu_t *imu, const gsof_data_t *pos, const prcopt_t *popt, const solopt_t *solopt, int port,
                  const char *file)
{
    rtk_t rtk = {0};
    int n = 0;
    ...
    if (fbfilt(imu, pos, popt, solopt, &rtk, 0))
        n = fbfilt(imu, pos, popt, solopt, &rtk, 1);
    if (n == 0)
    {
