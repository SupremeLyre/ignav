...
/* print imu measurement data-------------------------------------------------*/
static void primudata(vt_t *vt, const raw_t *raw)
{
    int i;
    const imu_t *pimu = &raw->imut;

    trace(4, "primudata:\n");

    if (raw->time.time == 0 || pimu->n == 0)
    {
        vt_printf(vt, "no imu raw data\n", ESC_BOLD);
        return;
    }
    prtime(vt, raw->time);

    vt_printf(vt, "\n%3s %10s %6s %6s %6s %6s %6s %6s %6s\n", "No.", "   timeTag", "Accx", "Accy", "Accz", "Gyrx",
              "Gyry", "Gyrz", "Temp");
    for (i = 0; i < pimu->n; i++)
    {
        vt_printf(vt, "%3d %10d %6.3lf %6.3lf %6.3lf %6.3lf %6.3lf %6.3lf %6.3lf\n", i + 1, pimu->data[i].imuc,
                  pimu->data[i].accl[0], pimu->data[i].accl[1], pimu->data[i].accl[2], pimu->data[i].gyro[0],
                  pimu->data[i].gyro[1], pimu->data[i].gyro[2], pimu->data[i].temp);
    }
    vt_printf(vt, "\n");
}
/* print solution ------------------------------------------------------------*/
...
static void probserv(vt_t *vt, int nf)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        for (j = 0; j < nf; j++)
            vt_printf(vt, "%3.0f", obs[i].SNR[j] * 0.25);
        for (j = 0; j < nf; j++)
...
static void prnavidata(vt_t *vt)
{
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        if (!(satsys(i + 1, &prn) & SYS_GLO) || geph[prn - 1].sat != i + 1)
            continue;
        valid =
...
static void prstream(vt_t *vt)
{
    ...
    const char *type[] = {"-",      "serial", "file", "tcpsvr", "tcpcli",   "udp",
                          "ntrips", "ntripc", "ftp",  "http",   "ntripc_s", "ntripc_c"};
    const char *fmt[] = {"rtcm2", "rtcm3", "oem4", "oem3", "ubx", "ss2", "hemis", "skytreq", "gw10", "javad",
                         "nvs",   "binex", "rt17", "sbf",  "cmr", "",    "",      "sp3",     ""};
...
/* imu measurement data command-----------------------------------------------*/
static void cmd_imudata(char **args, int narg, vt_t *vt)
{
    int cycle = 0;
...
static void *con_thread(void *arg)
{
    const char *cmds[] = {"start",    "stop",   "restart", "solution", "status", "satellite", "observ",
                          ...
                          "save",     "log",    "help",    "?",        "exit",   "shutdown",  "imudata",
                          "pause",    "resume", "reinit",  ""};
    con_t *con = (con_t *)arg;
    int i, j, narg;
    ...
    vt_printf(con->vt, "\n%s** %s ver.%s %s console (h:help) **%s\n", ESC_BOLD, PRGNAME, VER_RTKLIB, PATCH_LEVEL,
              ESC_RESET);
