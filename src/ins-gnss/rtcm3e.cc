...
{
    int prn;
    if (satsys(sat, &prn) != SYS_GLO || rtcm->nav.geph[prn - 1].sat != sat)
        return -1;
...
/* encode ssr header ---------------------------------------------------------*/
static int encode_ssr_head(int type, rtcm_t *rtcm, int sys, int nsat, int sync, int iod, double udint, int refd,
                           int provid, int solid)
{
    ...
    {
    case SYS_GPS:
        msgno = (type == 7) ? 2065 : 1056 + type;
        break;
    case SYS_GLO:
        msgno = (type == 7) ? 2066 : 1062 + type;
        break;
    case SYS_GAL:
        msgno = (type == 7) ? 2067 : 1239 + type;
        break;
    case SYS_QZS:
        msgno = (type == 7) ? 2068 : 1245 + type;
        break;
    case SYS_CMP:
        msgno = (type == 7) ? 2070 : 1257 + type;
        break;
    case SYS_SBS:
        msgno = (type == 7) ? 2069 : 1251 + type;
        break;
    default:
        return 0;
    }
...
static const int codes_gps[] = {CODE_L1C, CODE_L1P, CODE_L1W, CODE_L1Y, CODE_L1M, CODE_L2C,
                                CODE_L2D, CODE_L2S, CODE_L2L, CODE_L2X, CODE_L2P, CODE_L2W,
                                CODE_L2Y, CODE_L2M, CODE_L5I, CODE_L5Q, CODE_L5X};
static const int codes_glo[] = {CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P};
static const int codes_gal[] = {CODE_L1A, CODE_L1B, CODE_L1C, CODE_L1X, CODE_L1Z, CODE_L5I, CODE_L5Q,
                                CODE_L5X, CODE_L7I, CODE_L7Q, CODE_L7X, CODE_L8I, CODE_L8Q, CODE_L8X,
                                CODE_L6A, CODE_L6B, CODE_L6C, CODE_L6X, CODE_L6Z};
static const int codes_qzs[] = {CODE_L1C, CODE_L1S, CODE_L1L, CODE_L2S, CODE_L2L, CODE_L2X, CODE_L5I,
                                CODE_L5Q, CODE_L5X, CODE_L6S, CODE_L6L, CODE_L6X, CODE_L1X};
static const int codes_bds[] = {CODE_L1I, CODE_L1Q, CODE_L1X, CODE_L7I, CODE_L7Q,
...
static int encode_ssr7(rtcm_t *rtcm, int sys, int sync)
{
    ...
    for (j = nsat = 0; j < MAXSAT; j++)
    {
        ...
        for (k = nbias = 0; k < ncode; k++)
        {
            if (rtcm->ssr[j].pbias[codes[k] - 1] == 0.0)
                continue;
            code[nbias] = k;
...
static void gen_msm_index(rtcm_t *rtcm, int sys, int *nsat, int *nsig, int *ncell, unsigned char *sat_ind,
                          unsigned char *sig_ind, unsigned char *cell_ind)
{
    ...
    *nsat = *nsig = *ncell = 0;
    ...
    for (i = 0; i < rtcm->obs.n; i++)
    {
        ...
        {
            if (!(sig = to_sigid(sys, rtcm->obs.data[i].code[j], &f)))
                continue;
...
static void gen_msm_sat(rtcm_t *rtcm, int sys, int nsat, const unsigned char *sat_ind, double *rrng, double *rrate,
                        unsigned char *info)
{
    obsd_t *data;
    ...
    for (i = 0; i < rtcm->obs.n; i++)
    {
        ...
        for (j = 0; j < NFREQ + NEXOBS; j++)
        {
            if (!(sig = to_sigid(sys, data->code[j], &f)))
                continue;
            k = sat_ind[sat - 1] - 1;
...
static void gen_msm_sig(rtcm_t *rtcm, int sys, int nsat, int nsig, int ncell, const unsigned char *sat_ind,
                        const unsigned char *sig_ind, const unsigned char *cell_ind, const double *rrng,
                        const double *rrate, double *psrng, double *phrng, double *rate, int *lock, unsigned char *half,
                        float *cnr)
{
    obsd_t *data;
...
static int encode_msm_head(int type, rtcm_t *rtcm, int sys, int sync, int *nsat, int *ncell, double *rrng,
                           double *rrate, unsigned char *info, double *psrng, double *phrng, double *rate, int *lock,
                           unsigned char *half, float *cnr)
{
    double tow;
    ...
    setbitu(rtcm->buff, i, 7, to_lock(tt));
    i += 7; /* session time indicator */
    setbitu(rtcm->buff, i, 2, 0);
...
static int encode_msm_info(rtcm_t *rtcm, int i, const unsigned char *info, int nsat)
{
    int j;
...
static int encode_msm_half_amb(rtcm_t *rtcm, int i, const unsigned char *half, int ncell)
{
    int j;
...
extern int encode_rtcm3(rtcm_t *rtcm, int type, int sync)
{
    ...
    case 1047:
        ret = encode_type1047(rtcm, sync);
        break; /* beidou ephemeris (tentative mt) */
    case 63:
        ret = encode_type63(rtcm, sync);
        break; /* beidou ephemeris (rtcm draft) */
    case 1057:
        ret = encode_ssr1(rtcm, SYS_GPS, sync);
        break;
    case 1058:
        ret = encode_ssr2(rtcm, SYS_GPS, sync);
        break;
    case 1059:
        ret = encode_ssr3(rtcm, SYS_GPS, sync);
        break;
    case 1060:
        ret = encode_ssr4(rtcm, SYS_GPS, sync);
        break;
    case 1061:
        ret = encode_ssr5(rtcm, SYS_GPS, sync);
        break;
    case 1062:
        ret = encode_ssr6(rtcm, SYS_GPS, sync);
        break;
    case 1063:
        ret = encode_ssr1(rtcm, SYS_GLO, sync);
        break;
    case 1064:
        ret = encode_ssr2(rtcm, SYS_GLO, sync);
        break;
    case 1065:
        ret = encode_ssr3(rtcm, SYS_GLO, sync);
        break;
    case 1066:
        ret = encode_ssr4(rtcm, SYS_GLO, sync);
        break;
    case 1067:
        ret = encode_ssr5(rtcm, SYS_GLO, sync);
        break;
    case 1068:
        ret = encode_ssr6(rtcm, SYS_GLO, sync);
        break;
    case 1071:
    ...
    case 1127:
        ...
        break;
    case 1240:
        ret = encode_ssr1(rtcm, SYS_GAL, sync);
        break;
    case 1241:
        ret = encode_ssr2(rtcm, SYS_GAL, sync);
        break;
    case 1242:
        ret = encode_ssr3(rtcm, SYS_GAL, sync);
        break;
    case 1243:
        ret = encode_ssr4(rtcm, SYS_GAL, sync);
        break;
    case 1244:
        ret = encode_ssr5(rtcm, SYS_GAL, sync);
        break;
    case 1245:
        ret = encode_ssr6(rtcm, SYS_GAL, sync);
        break;
    case 1246:
        ret = encode_ssr1(rtcm, SYS_QZS, sync);
        break;
    case 1247:
        ret = encode_ssr2(rtcm, SYS_QZS, sync);
        break;
    case 1248:
        ret = encode_ssr3(rtcm, SYS_QZS, sync);
        break;
    case 1249:
        ret = encode_ssr4(rtcm, SYS_QZS, sync);
        break;
    case 1250:
        ret = encode_ssr5(rtcm, SYS_QZS, sync);
        break;
    case 1251:
        ret = encode_ssr6(rtcm, SYS_QZS, sync);
        break;
    case 1252:
        ret = encode_ssr1(rtcm, SYS_SBS, sync);
        break;
    case 1253:
        ret = encode_ssr2(rtcm, SYS_SBS, sync);
        break;
    case 1254:
        ret = encode_ssr3(rtcm, SYS_SBS, sync);
        break;
    case 1255:
        ret = encode_ssr4(rtcm, SYS_SBS, sync);
        break;
    case 1256:
        ret = encode_ssr5(rtcm, SYS_SBS, sync);
        break;
    case 1257:
        ret = encode_ssr6(rtcm, SYS_SBS, sync);
        break;
    case 1258:
        ret = encode_ssr1(rtcm, SYS_CMP, sync);
        break;
    case 1259:
        ret = encode_ssr2(rtcm, SYS_CMP, sync);
        break;
    case 1260:
        ret = encode_ssr3(rtcm, SYS_CMP, sync);
        break;
    case 1261:
        ret = encode_ssr4(rtcm, SYS_CMP, sync);
        break;
    case 1262:
        ret = encode_ssr5(rtcm, SYS_CMP, sync);
        break;
    case 1263:
        ret = encode_ssr6(rtcm, SYS_CMP, sync);
        break;
    case 2065:
        ret = encode_ssr7(rtcm, SYS_GPS, sync);
        break; /* tentative */
    case 2066:
        ret = encode_ssr7(rtcm, SYS_GLO, sync);
        break; /* tentative */
    case 2067:
        ret = encode_ssr7(rtcm, SYS_GAL, sync);
        break; /* tentative */
    case 2068:
        ret = encode_ssr7(rtcm, SYS_QZS, sync);
        break; /* tentative */
    case 2070:
        ret = encode_ssr7(rtcm, SYS_CMP, sync);
        break; /* tentative */
    }
    if (ret > 0)
    {
