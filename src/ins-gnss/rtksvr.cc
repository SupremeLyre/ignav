...
static void updatepvt(rtksvr_t *svr, const sol_t *sol, int isol)
{
    ...
    if (isol >= MAXSOLBUF)
        return;
    if (fabs(timediff(svr->pvt[isol - 1].time, sol->time)) < DTTOL && isol >= 1)
        return;
...
static void updateimg(rtksvr_t *svr, const img_t *img, int iimg)
{
    ...
    if (iimg >= MAXIMGBUF)
        return;
    if (fabs(timediff(svr->img[iimg - 1].time, img->time)) < DTTOL && iimg >= 1)
        return;
...
static void updatepose(rtksvr_t *svr, const pose_meas_t *pose, int ipose)
{
    ...
    if (ipose >= MAXPOSEBUF)
        return;
    if (ipose >= 1 && fabs(timediff(svr->pose[ipose - 1].time, svr->pose[ipose].time)) < DTTOL)
    {
        return;
    }
    svr->pose[ipose] = *pose;
...
static int inputobstc(rtksvr_t *svr, gtime_t time, obsd_t *obs)
{
    ...
    for (i = 0, dt = 0.0, n = NS(svr->syn.rover, svr->syn.nr, MAXOBSBUF); i < n + 1 && svr->syn.nr; i++)
    {
        j = svr->syn.rover + i;
        ...
        if (fabs((dt = timediff(time, robs[j].data[0].time))) < DTTOL && robs[j].n != 0)
        {
            for (k = 0, m = 0; k < robs[j].n; k++)
                obs[m++] = robs[j].data[k];
            svr->syn.rover = j;
            tr = obs[0].time;
            flag = 1;
    ...
    if (tr.time)
    {
        for (i = 0, dt = 0.0, n = NS(svr->syn.base, svr->syn.nb, MAXOBSBUF); i < n + 1 && svr->syn.nb; i++)
        {
            j = svr->syn.base + i;
            ...
            if (fabs((dt = timediff(tr, bobs[j].data[0].time))) < DTTOLM && bobs[j].n != 0)
            {
                for (k = 0; k < bobs[j].n; k++)
                    obs[m++] = bobs[j].data[k];
                svr->syn.base = j;
                flag = 2;
                break;
...
static int inputimu(rtksvr_t *svr, imud_t *data)
{
    ...
    for (i = 0, k = 0, n = NS(svr->syn.imu, svr->syn.ni, MAXIMUBUF), j = svr->syn.imu; i < n + 1; i++)
    {
        j = svr->syn.imu + i;
        if (j >= MAXIMUBUF)
            j = j % MAXIMUBUF;
...
static int inputpvt(rtksvr_t *svr, gtime_t t0, sol_t *sol)
{
    ...
    *sol = sol0;
    ...
    for (i = 0, dt = 0.0, n = NS(svr->syn.pvt, svr->syn.ns, MAXSOLBUF); i < n + 1 && svr->syn.ns; i++)
    {
        j = svr->syn.pvt + i;
        ...
        if (fabs((dt = timediff(t0, svr->pvt[j].time))) < DTTOL && svr->pvt[j].time.time != 0 &&
...
static int inputimg(rtksvr_t *svr, gtime_t t0, img_t **img)
{
    ...
    for (i = 0, dt = 0.0, n = NS(svr->syn.img, svr->syn.nm, MAXIMGBUF); i < n + 1 && svr->syn.nm; i++)
    {
        j = svr->syn.img + i;
        ...
        if (fabs((dt = timediff(t0, svr->img[j].time))) < DTTOL && svr->img[j].flag == 0 && svr->img[j].h &&
            svr->img[j].w && svr->img[j].time.time != 0)
        {
            *img = &svr->img[j];
            svr->syn.img = j;
            return 1; /* ok */
...
static int inputimage(rtksvr_t *svr, img_t *data)
{
    ...
    for (i = 0, k = 0, n = NS(svr->syn.img, svr->syn.nm, MAXIMGBUF), j = svr->syn.img; i < n + 1; i++)
    {
        j = svr->syn.img + i;
        if (j >= MAXIMGBUF)
            j = j % MAXIMGBUF;
...
static int inputpose(rtksvr_t *svr, gtime_t time, pose_meas_t *pose)
{
    ...
    for (i = 0; i < np + 1; i++)
    {
        ...
        if (fabs((dt = timediff(time, svr->pose[j].time))) < DTTOL && svr->pose[j].time.time && svr->pose[j].stat &&
...
static int decoderaw(rtksvr_t *svr, int index)
{
    ...
    case 0:
        fobs = svr->syn.nr;
        svr->syn.nrp = svr->syn.nr;
        break;
    case 1:
        fobs = svr->syn.nb;
        svr->syn.nbp = svr->syn.nb;
        break;
    case 3:
        fobs = svr->syn.ns;
        svr->syn.nsp = svr->syn.ns;
        break;
    case 4:
        fobs = svr->syn.ni;
        svr->syn.nip = svr->syn.ni;
        break;
    case 5:
        fobs = svr->syn.nm;
        svr->syn.nmp = svr->syn.nm;
        break;
    case 6:
        fobs = svr->syn.np;
        svr->syn.npp = svr->syn.np;
        break;
    }
    for (i = 0; i < svr->nb[index]; i++)
    {
        ...
        if (ret == 1 && obs->n)
        {
            ...
            if (k < MAXOBSBUF)
                ...
                svr->prcout++;
            if (fobs >= MAXOBSBUF && index == 0)
                svr->syn.of[0] = fobs / MAXOBSBUF;
            if (fobs >= MAXOBSBUF && index == 1)
                svr->syn.of[1] = fobs / MAXOBSBUF;
        }
        /* update ephemeris */
        if (ret == 2)
        {
        ...
        if (ret == 32 && (index == 0 || index == 1))
        {
            ...
            for (j = 0; j < 7; j++)
                svr->rtk.opt.sind[0][j] = svr->raw[0].rinex.index[j];
            for (j = 0; j < 7; j++)
                svr->rtk.opt.sind[1][j] = svr->raw[1].rinex.index[j];

            for (j = 0; j < 7; j++)
                svr->nav.sind[0][j] = svr->raw[0].rinex.index[j];
            for (j = 0; j < 7; j++)
                svr->nav.sind[1][j] = svr->raw[1].rinex.index[j];
        }
    ...
    {
        if (index == 0)
            svr->syn.time[0] = svr->obs[0][(fobs - 1) % MAXOBSBUF].data[0].time;
        if (index == 1)
            svr->syn.time[1] = svr->obs[1][(fobs - 1) % MAXOBSBUF].data[0].time;
        if (index == 3)
            svr->syn.time[3] = svr->pvt[(fobs - 1) % MAXSOLBUF].time;
        if (index == 4)
            svr->syn.time[2] = svr->imu[(fobs - 1) % MAXIMUBUF].time;
        if (index == 5)
            svr->syn.time[4] = svr->img[(fobs - 1) % MAXIMGBUF].time;
        if (index == 6)
            svr->syn.time[5] = svr->pose[(fobs - 1) % MAXPOSEBUF].time;
    }
    switch (index)
    {
    case 0:
        k = MIN(NE(fobs % MAXOBSBUF, svr->syn.rover, MAXOBSBUF), MAXOBS);
        break;
    case 1:
        k = MIN(NE(fobs % MAXOBSBUF, svr->syn.base, MAXOBSBUF), MAXOBS);
        break;
    case 3:
        k = MIN(NE(fobs % MAXSOLBUF, svr->syn.pvt, MAXSOLBUF), MAXSOL);
        break;
    case 4:
        k = MIN(NE(fobs % MAXIMUBUF, svr->syn.imu, MAXIMUBUF), MAXIMU);
        break;
    case 5:
        k = MIN(NE(fobs % MAXIMGBUF, svr->syn.img, MAXIMGBUF), MAXIMG);
        break;
    case 6:
        k = MIN(NE(fobs % MAXPOSEBUF, svr->syn.pose, MAXPOSEBUF), MAXPOSE);
        break;
    }
    svr->nb[index] = 0;
    rtksvrunlock(svr);
...
static void updatetimediff(rtksvr_t *svr)
{
    ...
    if (syn->nm && syn->ni)
    {
        syn->dt[2] = time2gpst(syn->time[2], NULL) - time2gpst(syn->time[4], NULL);
    }
...
static int solimualign(rtksvr_t *svr)
{
    ...
    for (i = 0; i < (syn->of[2] ? MAXIMUBUF : syn->ni) && !syn->tali[1]; i++)
    {
        ...
        for (j = 0; j < (syn->of[3] ? MAXSOLBUF : syn->ns); j++)
        {
            ...
            if (sow1 == 0.0 || sow2 == 0.0 || fabs(sow1 - sow2) > DTTOL)
                continue;
...
static int imuobsalign(rtksvr_t *svr)
{
    ...
    for (i = 0; i < n && svr->syn.tali[2] != 2; i++)
...
static int imuimgalign(rtksvr_t *svr)
{
    ...
    for (i = 0; i < (psyn->of[2] ? MAXIMUBUF : psyn->ni) && svr->syn.tali[3] != 1; i++)
    {
        ...
        for (j = 0; j < (psyn->of[4] ? MAXIMGBUF : psyn->nm); j++)
        {
            ...
            if (svr->img[j].flag || svr->img[j].h == 0 || svr->img[j].w == 0)
                continue;
            if (fabs(sow1 - sow2) > DTTOL)
                continue;
...
{
    if (svr->rtk.opt.mode <= PMODE_FIXED)
        return rbobsalign(svr);

    if (svr->rtk.opt.mode == PMODE_INS_LGNSS)
    {
        if (svr->rtk.opt.insopt.lcopt == IGCOM_USEOBS)
            return imuobsalign(svr);
        if (svr->rtk.opt.insopt.lcopt == IGCOM_USESOL)
            return solimualign(svr);
    }
    if (svr->rtk.opt.mode == PMODE_INS_TGNSS)
        return imuobsalign(svr);
    if (svr->rtk.opt.mode == PMODE_INS_LVO)
...
static void motion(const insopt_t *opt, imud_t *imuz, insstate_t *ins, imud_t *imu, int ws)
{
    static int i, nc = 0, zf = 0;
    double pos[3];
...
#endif
{
    ...
    for (cycle = 0; svr->state; cycle++)
    {
        ...
        if (fobs[1] > 0 && svr->rtk.opt.rb[0] == 0.0 && svr->rtk.opt.refpos == POSOPT_SINGLE)
        {
        ...
        if (opt->mode == PMODE_INS_TGNSS)
            for (j = INSUPD_TIME, i = 0; i < imus.n; i++, j = INSUPD_TIME)
            {
        ...
        if (opt->mode == PMODE_VO)
        {
            ...
            updatevo:
                ...
                rtksvrlock(svr);
                flag = estvo(&opt->insopt.voopt, &imgs.data[i], svr->rtk.sol.dT, svr->rtk.sol.vc, svr->rtk.sol.var);
...
extern void rtksvrfree(rtksvr_t *svr)
{
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < MAXOBSBUF; j++)
        {
            ...
            svr->obs[i][j].data = NULL;
        }
    if (svr->rtk.ins.rtkp)
        rtkfree((rtk_t *)svr->rtk.ins.rtkp);
    rtkfree(&svr->rtk);
}
...
extern int rtksvrmark(rtksvr_t *svr, const char *name, const char *comment)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if (svr->solopt[i].posf == SOLF_STAT)
        {
            p += sprintf(p, "$MARK,%d,%.3f,%d,%.4f,%.4f,%.4f,%s,%s\n", week, tow, svr->rtk.sol.stat, svr->rtk.sol.rr[0],
                         svr->rtk.sol.rr[1], svr->rtk.sol.rr[2], name, comment);
        }
