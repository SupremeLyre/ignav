...
#ifdef EXTGSI
extern int resamb_WLNL(rtk_t *rtk, const obsd_t *obs, const int *sat, const int *iu, const int *ir, int ns,
                       const nav_t *nav, const double *azel);
extern int resamb_TCAR(rtk_t *rtk, const obsd_t *obs, const int *sat, const int *iu, const int *ir, int ns,
                       const nav_t *nav, const double *azel);
#else
extern int resamb_WLNL(rtk_t *rtk, const obsd_t *obs, const int *sat, const int *iu, const int *ir, int ns,
                       const nav_t *nav, const double *azel)
...
extern int insoutstat(rtk_t *rtk, char *buff)
{
    ...
    p += sprintf(p, "$VELACCI,%d,%.3f,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n", week, tow, rtk->sol.stat, vel[0], vel[1],
                 vel[2], acc[0], acc[1], acc[2]);

    /* imu body attitude */
    p += sprintf(p, "$ATT,%d,%.3f,%d,%.4f,%.4f,%.4f\n", week, tow, rtk->sol.stat, rtk->sol.att[0] * R2D,
                 rtk->sol.att[1] * R2D, rtk->sol.att[2] * R2D);

    /* gyro/accl bias */
    p += sprintf(p, "$GBIAS,%d,%.3f,%d,%.4f,%.4f,%.4f\n", week, tow, rtk->sol.stat, rtk->sol.bg[0], rtk->sol.bg[1],
                 rtk->sol.bg[2]);
    p += sprintf(p, "$ABIAS,%d,%.3f,%d,%.4f,%.4f,%.4f\n", week, tow, rtk->sol.stat, rtk->sol.ba[0], rtk->sol.bg[1],
                 rtk->sol.bg[2]);
...
static void outsolstat(rtk_t *rtk)
{
    ...
    if (rtk->opt.mode < PMODE_INS_UPDATE)
        n = rtkoutstat(rtk, buff);
    if (rtk->opt.mode >= PMODE_INS_UPDATE)
        n = insoutstat(rtk, buff);
    buff[n] = '\0';

    fputs(buff, fp_stat);

    if ((rtk->sol.stat == SOLQ_NONE && rtk->ins.stat == INSS_NONE) || statlevel <= 1)
        return;
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        for (j = 0; j < nfreq; j++)
        {
            fprintf(fp_stat, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.0f,%d,%d,%d,%d,%d,%d\n", week, tow, id, j + 1,
                    ...
                    ssat->snr[j] * 0.25, ssat->fix[j], ssat->slip[j] & 3, ssat->lock[j], ssat->outc[j], ssat->slipc[j],
                    ssat->rejc[j]);
        }
    }
}
...
static double sdobspins(const rtk_t *rtk, const obsd_t *obs, const double *rs, int i, int j, int f)
{
    double rr[3], e[3], sdg, sdi, ri, rj;

    sdg = sdobs(obs, i, j, f);
    if (rtk->opt.mode != PMODE_INS_TGNSS)
        return sdg;

    insp2antp(&rtk->ins, rr);

    if ((ri = geodist(rs + 6 * i, rr, e)) <= 0.0)
        return sdg;
    if ((rj = geodist(rs + 6 * j, rtk->rb, e)) <= 0.0)
        return sdg;
...
static void udion(rtk_t *rtk, double tt, double bl, const int *sat, int ns)
{
    ...
    for (i = 1; i <= MAXSAT; i++)
    {
        ...
        if (x[j] != 0.0 && rtk->ssat[i - 1].outc[0] > GAP_RESION && rtk->ssat[i - 1].outc[1] > GAP_RESION)
            x[j] = 0.0;
    }
    for (i = 0; i < ns; i++)
    {
        ...
        if (x[j] == 0.0)
        {
            tc ? insinitx(&rtk->ins, 1E-6, SQR(rtk->opt.std[1] * bl / 1E4), j)
        ...
        else
        {
            ...
            fact = cos(el);
            tc ? ins->P[j + j * ins->nx] += SQR(rtk->opt.prn[1] * bl / 1E4 * fact) * fabs(tt)
...
static void udtrop(rtk_t *rtk, double tt, double bl)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if (x[j] == 0.0)
        {
            tc ? insinitx(ins, INIT_ZWD, SQR(rtk->opt.std[2]), j)
            ...
            if (rtk->opt.tropopt >= TROPOPT_ESTG)
            {
                for (k = 0; k < 2; k++)
                {
                    tc ? insinitx(&rtk->ins, 1E-6, VAR_GRA, ++j) : initx(rtk, 1E-6, VAR_GRA, ++j);
                }
            }
        }
        else
...
static void udrcvbias(rtk_t *rtk, double tt)
{
    ...
    for (i = 0; i < NFREQGLO; i++)
    {
        ...
        if (x[j] == 0.0)
        {
            tc ? insinitx(&rtk->ins, 1E-6, VAR_HWBIAS, j) : initx(rtk, 1E-6, VAR_HWBIAS, j);
        }
        /* hold to fixed solution */
        else if (rtk->nfix >= rtk->opt.minfix && rtk->sol.ratio > rtk->opt.thresar[0])
        {
            tc ? insinitx(&rtk->ins, rtk->ins.xb[j], rtk->ins.Pb[j + j * rtk->ins.nx], j)
...
static double mwmeas(const obsd_t *obs, int iu, int ir, const nav_t *nav)
{
    ...
    if (obs[ir].L[0] == 0.0 || obs[iu].L[0] == 0.0)
        return 0.0;
    if (obs[ir].P[i] == 0.0 || obs[iu].P[i] == 0.0)
        return 0.0;
    ...
    return lam[0] * lam[i] * (l1 - l2) / (lam[i] - lam[0]) - (lam[i] * p1 + lam[0] * p2) / (lam[i] + lam[0]);
}
...
static void udbias(rtk_t *rtk, double tt, const obsd_t *obs, const int *sat, const double *rs, const int *iu,
    ...
    for (f = 0; f < nf; f++)
    {
        ...
        for (i = 1; i <= MAXSAT; i++)
        {
            ...
            if (tc && rtk->ins.ptct.time != 0)
            {
                ...
                rtk->ssat[i - 1].outc[f] += ROUND(rtk->tt / eps);
            ...
            {
                tc ? insinitx(&rtk->ins, 0.0, SQR(rtk->opt.std[0]), ib) : initx(rtk, 0.0, SQR(rtk->opt.std[0]), ib);
            }
        ...
        for (i = 0; i < ns; i++)
        {
            ...
            if (bias[i] == 0.0 || x[ib] != 0.0)
                continue;
            tc ? insinitx(&rtk->ins, bias[i], SQR(rtk->opt.std[0]), ib) : initx(rtk, bias[i], SQR(rtk->opt.std[0]), ib);
        }
        free(bias);
    }
}
...
static int zdres(int base, const obsd_t *obs, int n, const double *rs, const double *dts, const int *svh,
                 const nav_t *nav, const double *rr, const prcopt_t *opt, int index, double *y, double *e, double *azel)
{
...
static void jacob_dd_dp(const insstate_t *ins, const double *ei, const double *ej, double *dddp)
{
    dddp[0] = ei[0] - ej[0];
    dddp[1] = ei[1] - ej[1];
    dddp[2] = ei[2] - ej[2];
...
static void jacob_dd_da(const insstate_t *ins, const double *ei, const double *ej, double *ddda)
{
#if NOINSJACO
...
static void jacob_dd_dl(const insstate_t *ins, const double *ei, const double *ej, double *dddl)
{
#if NOINSJACO
    return; /* no add jacobians */
#else
...
static int ddres(rtk_t *rtk, const nav_t *nav, const obsd_t *obs, double dt, const double *x, const double *P,
                 const int *sat, double *y, double *e, double *azel, const int *iu, const int *ir, int ns, double *v,
                 double *H, double *R, int *vflg, const double *re, double *Ri_, double *Rj_, int *nb_, int *b_,
    ...
    if (nf >= 2 && opt->mode > PMODE_DGPS)
    {
        ...
        for (m = 0, flag = 1; m < ns; m++, flag = 1)
        {
            for (l = 0, i = 0; i < ns; i++)
            {
                ...
                if (rtk->refsat[sysind(rtk->ssat[sat[i] - 1].sys)][0] == sat[i])
                    continue;
                if (rtk->refsat[sysind(rtk->ssat[sat[i] - 1].sys)][1] == sat[i])
                    continue;
...
static int ddmat_WL(int na, int nb, const ddsat_t *ddsat, double *D, int *index, ddsat_t *wlsat)
{
    ...
    for (k = 0, i = 0; i < nb; i++)
    {
        ...
        for (j = 0; j < nb; j++)
        {
            if (flag[j] && i != j && ddsat[j].sat1 == ddsat[i].sat1 && ddsat[j].sat2 == ddsat[i].sat2 &&
                ddsat[j].f == 1)
            {
...
static ddamb_t *getddamb(amb_t *bias, int sat1, int sat2, int f)
{
    int i;
    for (i = 0; i < bias->nb; i++)
        if (sat1 == bias->amb[i].sat1 && sat2 == bias->amb[i].sat2 && f == bias->amb[i].f)
            return bias->amb + i;
    return NULL;
}
...
static int ddmat(rtk_t *rtk, double *D, ddsat_t *ddsat, const int *vflg, int nv, int flag)
{
    ...
    for (i = 0; i < nv; i++)
    {
        ...
        if ((m == 1 && rtk->opt.glomodear == 0) || (m == 3 && rtk->opt.bdsmodear == 0))
        {
            continue;
        }
        ...
        if ((rtk->ssat[sat2 - 1].lock[f] >= 0 || flag) && rtk->ssat[sat2 - 1].vsat[f] &&
            !(rtk->ssat[j - k].slip[f] & 2) && rtk->ssat[sat2 - 1].azel[1] >= rtk->opt.elmaskar)
        {
            ...
            rtk->ssat[sat2 - 1].fix[f] = 1; /* no fix */
    }
    trace(5, "D=\n");
...
static void restamb(rtk_t *rtk, const double *bias, ddsat_t *ddsat, int nb, double *xa)
{
    ...
    for (i = 0; i < nx; i++)
        tc ? xa[i] = rtk->ins.x[i] : xa[i] = rtk->x[i];
    for (i = 0; i < na; i++)
        tc ? xa[i] = rtk->ins.xb[i] : xa[i] = rtk->xa[i];
...
static void holdamb(rtk_t *rtk, insstate_t *ins, const double *xa, const ddsat_t *ddsat, int ns)
{
    ...
    for (i = 0; i < ns; i++)
    {
        ...
        if (rtk->ssat[sat2 - 1].azel[1] < rtk->opt.elmaskhold)
            continue;
    ...
    if (nv > 0)
    {
        R = zeros(nv, nv);
        for (i = 0; i < nv; i++)
            R[i + i * nv] = (rtk->bias.inherit ? VAR_HOLDAMB * 10.0 : VAR_HOLDAMB);

        /* close-loop states set to zero */
        if (tc)
        {
            for (i = 0; i < xnCl(&rtk->opt.insopt); i++)
                x[i] = 0.0;
        }
...
static int inheritambwl(rtk_t *rtk, const ddsat_t *wlsat, const double *wl, int nw, double *b, double *s)
{
    ddamb_t *pamb = NULL;
    int i, k = 0;
    ...
    for (i = 0; i < nw; i++)
    {
        if ((pamb = getddamb(&rtk->wlbias, wlsat[i].sat1, wlsat[i].sat2, wlsat[i].f)) == NULL)
            continue;
        if (pamb->ratio < rtk->opt.thresar[0] * 3.0)
            continue;

        if (timediff(rtk->sol.time, pamb->time) > THRES_INHERIT_TIME)
            continue;
        if (fabs(wl[i] - pamb->bias) > THRES_INHERIT_BIAS)
            continue;
        b[i] = pamb->bias;
        k++;
    }
...
static int resamb_WL(rtk_t *rtk, double *Qy, double *y, int ny, int *index, const double *D, int nw, ddsat_t *wlsat)
{
    ...
    if (!lambda(nw, 2, wl + na, Qw, b, s))
    {
        ...
        if ((s[0] <= 0.0 || s[1] / s[0] >= rtk->opt.thresar[0]) ||
            (inherit = inheritambwl(rtk, wlsat, wl + na, nw, b, s)))
        {
            if (inherit)
            {
...
/* inherit ambiguity---------------------------------------------------------*/
static int inheritamb(rtk_t *rtk, ddsat_t *ddsat, const double *y, const double *Qy, int na, int nb, double *b,
                      double *s)
{
    ddamb_t *pamb = NULL;
    int i, k = 0;
    ...
    {
        ddsat[i].flag = 1;
        if ((pamb = getddamb(&rtk->bias, ddsat[i].sat1, ddsat[i].sat2, ddsat[i].f)) == NULL)
            continue;
        if (pamb->ratio < rtk->opt.thresar[0] * 10.0)
            continue;

        if (timediff(rtk->sol.time, pamb->time) > THRES_INHERIT_TIME)
            continue;
        if (fabs(y[i] - pamb->bias) > THRES_INHERIT_BIAS)
            continue;
        ddsat[i].flag = 0;
        b[i] = pamb->bias;
        k++;
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa, ddsat_t *ddsat, int *namb, const int *vflg, int nv)
{
    prcopt_t *opt = &rtk->opt;
    ...
    *namb = 0;
    ...
    if (!(info = lambda(nb, 2, y + na, Qb, b, s)))
    {
        ...
        if ((s[0] <= 0.0 || s[1] / s[0] >= opt->thresar[0]) ||
            (inherit = inheritamb(rtk, ddsat, y + na, Qy, na, nb, b, s)))
        {
...
static int valpos(rtk_t *rtk, const double *v, const double *R, const int *vflg, int nv, int *m, double thres)
{
...
static int relpos(rtk_t *rtk, const obsd_t *obs, int nu, int nr, const nav_t *nav)
{
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        for (j = 1; j < NFREQ; j++)
            rtk->ssat[i].snr[j] = 0;
        for (j = 0; j < NFREQ; j++)
            rtk->ssat[i].vsatc[j] = 0;
    }
    /* satellite positions/clocks */
    satposs(time, obs, n, nav, opt->sateph, rs, dts, var, svh);

    /* undifferenced residuals for base station */
    if (!zdres(1, obs + nu, nr, rs + nu * 6, dts + nu * 2, svh + nu, nav, rtk->rb, opt, 1, y + nu * nf * 2, e + nu * 3,
               azel + nu * 2))
    {
    ...
    for (i = 0; i < (tc ? 1 : niter); i++)
    {
        ...
        if ((nv = ddres(rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, iu, ir, ns, v, H, R, vflg, rr, NULL, NULL, NULL,
                        NULL, refsat)) < 1)
        {
    ...
    if (stat != SOLQ_NONE && zdres(0, obs, nu, rs, dts, svh, nav, rr, opt, 0, y, e, azel))
    {
        ...
        nv =
            ddres(rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, iu, ir, ns, v, NULL, R, vflg, rr, Ri, Rj, nb, &b, refsat);
    ...
    else if (stat != SOLQ_NONE && resamb_LAMBDA(rtk, bias, xa, ddsat, &namb, vflg, nv) > 1)
    {
        ...
        if (zdres(0, obs, nu, rs, dts, svh, nav, rr, opt, 0, y, e, azel))
        {
            ...
            nv = ddres(rtk, nav, obs, dt, xa, NULL, sat, y, e, azel, iu, ir, ns, v, NULL, R, vflg, rr, NULL, NULL, NULL,
                       NULL, NULL);
    ...
    ix = tc ? xiP(insopt) + 0 : 0;
    iy = tc ? xiP(insopt) + 1 : 1;
    iz = tc ? xiP(insopt) + 2 : 2;
...
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)
{
    ...
    if (opt->mode >= PMODE_INS_UPDATE && opt->mode <= PMODE_INS_TGNSS)
    {
        ...
        if (opt->mode == PMODE_INS_LGNSS && opt->insopt.lcopt == IGCOM_USEOBS)
        {
            ...
            {
            case INSLC_SINGLE:
                rtk->opt.mode = PMODE_SINGLE;
                break;
            case INSLC_PPK:
                rtk->opt.mode = PMODE_PPP_KINEMA;
                break;
            case INSLC_DGPS:
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if (!pntpos(opt->adjobs ? obsd : obs, nu, nav, &rtk->opt, &rtk->sol, tcs ? &rtk->ins : NULL, NULL, rtk->ssat, msg))
    {
    ...
    if (fabs(rtk->tt) < DTTOL && opt->mode <= PMODE_FIXED)
        return stat;
