 ...
 * return : none
 * --------------------------------------------------------------------------*/
static void jaco_dop_dp(const double *rs, const double *rr, const double *vs, const double *vr, double *dopdp)
{
    int i;
 ...
 * return : none
 * --------------------------------------------------------------------------*/
static void jaco_dop_da(const double *dr, const double *Cbe, const double *l, const double *dopdv, const double *gyro,
                        double *dopda)
{
    int i;
    double T[9], sl[3], S[9];
...
static void jaco_dop_bg(const double *dopdv, const double *Cbe, const double *l, double *dopbg)
{
    double T[9], S[9];
...
static void jaco_dop_dl(const double *dopdv, const double *dr, const double *Cbe, const double *gyro, double *dopdl)
{
    int i;
    double T[9], S[9];
...
static int doppHVR(const prcopt_t *opt, const insstate_t *ins, const double *rs, const double *dts, const int *svh,
                   int n, const obsd_t *obs, const nav_t *nav, double *v, double *H, double *R)
{
    ...
    for (i = 0; i < n && i < MAXOBS; i++)
    {
        ...
        if (!satsys(obs[i].sat, NULL))
            continue;

        /* geometric distance/azimuth/elevation angle */
        if (geodist(rs + i * 6, rr, e) <= 0.0 || satazel(pos, e, azel) < opt->elmin)
            continue;
        ...
        rate =
            dot(vs, e, 3) +
            OMGE / CLIGHT * (rs[4 + i * 6] * rr[0] + rs[1 + i * 6] * vr[0] - rs[3 + i * 6] * rr[1] - rs[i * 6] * vr[1]);
...
#endif
                for (j = IP; j < IP + NP; j++)
                    H[j + nv * nx] = dopdp[j - IP];
                for (j = IA; j < IA + NA; j++)
                    H[j + nv * nx] = dopda[j - IA];
                for (j = IV; j < IV + NV; j++)
                    H[j + nv * nx] = dopdv[j - IV];
...
static int valdopp(const insopt_t *opt, const double *x, const double *R, const double *v, int nv, double thres)
{
...
static int doppfilt(insstate_t *ins, const prcopt_t *opt, const obsd_t *obs, int n, const nav_t *nav, const double *rs,
                    const double *dts, const int *svh)
{
    double *v, *H, *R, *x, *P;
 ...
 * --------------------------------------------------------------------------*/
extern int doppler(const obsd_t *obs, int n, const nav_t *nav, const prcopt_t *opt, insstate_t *ins)
{
    double *rs, *dts, *var;
