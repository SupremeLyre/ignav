...
static int code2sys(char code)
{
    ...
    if (code == 'C')
        return SYS_CMP; /* extension to sp3-c */
    if (code == 'L')
...
extern void readsp3(const char *file, nav_t *nav, int opt)
{
    ...
    for (i = j = 0; i < n; i++)
    {
        ...
        if (!strstr(ext + 1, "sp3") && !strstr(ext + 1, ".SP3") && !strstr(ext + 1, "eph") && !strstr(ext + 1, ".EPH"))
            continue;
...
/* add satellite fcb ---------------------------------------------------------*/
static int addfcb(nav_t *nav, gtime_t ts, gtime_t te, int sat, const double *bias, const double *std)
{
    fcbd_t *nav_fcb;
    int i, j;
...
extern void satantoff(gtime_t time, const double *rs, int sat, const nav_t *nav, double *dant)
{
    ...
    if (NFREQ >= 3 && (satsys(sat, NULL) & (SYS_GAL | SYS_SBS)))
        k = 2;
