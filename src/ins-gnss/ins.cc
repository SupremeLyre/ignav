...
/* D=A*B*C---------------------------------------------------------------------*/
extern void matmul33(const char *tr, const double *A, const double *B, const double *C, int n, int p, int q, int m,
                     double *D)
{
    char tr_[8];
    double *T = mat(n, q);
...
{
    double sin_phi = sin(rpy[0]), cos_phi = cos(rpy[0]), sin_theta = sin(rpy[1]), cos_theta = cos(rpy[1]),
...
extern void rfu2frd(const double *rfu, double *frd, double *C)
{
    ...
    if (C)
        setzero(C, 3, 3), C[1] = C[3] = 1.0, C[8] = -1.0;
}
 ...
 * return  :none
 * -----------------------------------------------------------------------*/
extern void ins_errmodel(const double *accl, const double *gyro, double *cor_accl, double *cor_gyro, insstate_t *ins)
{
    int i, j;
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            Mai[i + j * 3] = I[i + j * 3] + ins->Ma[i + j * 3];
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            Mgi[i + j * 3] = I[i + j * 3] + ins->Mg[i + j * 3];
 ...
 * return : none
 * -----------------------------------------------------------------------*/
extern void ins_errmodel2(const double *accl, const double *gyro, const double *Ma, const double *Mg, const double *ba,
                          const double *bg, const double *Gg, double *cor_accl, double *cor_gyro)
{
    int i, j;
    double Mai[9], Mgi[9], *I = eye(3), T[9] = {0}, Gf[3] = {0};
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            Mai[i + j * 3] = I[i + j * 3] + Ma[i + j * 3];
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            Mgi[i + j * 3] = I[i + j * 3] + Mg[i + j * 3];
...
extern void rov2dcm(const double *rv, double *C)
{
    ...
    if ((a = norm(rv, 3)) < 1E-8)
    {
        ...
        n1 = sin(a) / a, n2 = (1.0 - cos(a)) / SQR(a);

    skewsym3(rv, rs);
    matmul("NN", 3, 3, 3, 1.0, rs, rs, 0.0, rs2);
    for (i = 0; i < 9; i++)
        C[i] = I[i] - n1 * rs[i] + n2 * rs2[i];
    free(I);
}
...
extern void gravity_ned(const double *pos, double *gn)
{
    ...
    gn[2] = g0 * (1.0 - 2.0 / RE * (1.0 + FLAT * (1.0 - 2 * SQR(sin(pos[0]))) + SQR(OMGE * RE) * RP / MU) * pos[2] +
                  3.0 / SQR(RE) * SQR(pos[2])); /* (2.139)-v2.0 */
}
 ...
 * return: none
 * --------------------------------------------------------------------------*/
extern void rp2head(const double roll, const double pitch, const double *gyro, double *head)
{
    double sinh = -gyro[1] * cos(roll) + gyro[2] * sin(roll);
    double cosh = gyro[0] * cos(pitch) + gyro[1] * sin(roll) * sin(pitch) + gyro[2] * cos(roll) * sin(pitch);
 ...
 * return : none
 *----------------------------------------------------------------------------*/
extern void initins(insstate_t *ins, const double *re, double angh, const imud_t *data, int n, const insopt_t *opt)
{
    gtime_t time = {0};
 ...
 * --------------------------------------------------------------------------*/
extern void rmlever(const double *pos, const double *re, const double *ve, const double *lever, const double *Cbe,
                    const double *omgb, double *rec, double *vec)
{
    int i;
    double wl[3], T[3], Omg[9];
 ...
 * return :none---------------------------------------------------------------*/
extern void cnscl(const double *gyro, const double *accl, double *phim, double *dvbm, const double *wm0,
                  const double *vm0, double *dphim, double *rotm, double *scullm)
{
    ...
    cross3(wm0, gyro, tmp);
    for (i = 0; i < 3; i++)
        dphim[i] = tmp[i] * 1.0 / 12.0;

    for (i = 0; i < 3; i++)
        phim[i] = gyro[i] + dphim[i];

    cross3(gyro, accl, tmp);
    cross3(vm0, gyro, tmp1);
    for (i = 0; i < 3; i++)
        scullm[i] = (tmp[i] + tmp1[i]) / 12.0;

    cross3(gyro, accl, tmp);
    for (i = 0; i < 3; i++)
        rotm[i] = 1.0 / 2.0 * tmp[i];
    for (i = 0; i < 3; i++)
        dvbm[i] = accl[i] + rotm[i] + scullm[i];
}
/* trace ins states ----------------------------------------------------------*/
 ...
 *-----------------------------------------------------------------------------*/
extern int readimu(const char *file, imu_t *imu, int decfmt, int format, int coor, int valfmt)
{
    FILE *fp;
    imud_t *p;
    double v[12], sow;
 ...
 * return : none
 * -------------------------------------------------------------------------*/
extern void gapv2ipv(const double *pos, const double *vel, const double *Cbe, const double *lever, const imud_t *imu,
...
static void geoparam(const double *pos, const double *vn, double *wen_n, double *wie_n, double *g, double *Reo,
                     double *Rno)
{
...
/* rotational and sculling motion correction --------------------------------*/
extern void rotscull_corr(insstate_t *ins, const insopt_t *opt, double dt, double *dv, double *da)
{
