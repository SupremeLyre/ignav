...
static double undecli(gtime_t time, const double *pos)
{
    ...
    time2epoch(time, ep);
    get_field_components(&mag, &mag_model, pos[2], Units::kUnitsMeters, CoordinateSystem::kCoordSysGeodetic, pos[0],
                         pos[1], julday(ep[1], ep[2], ep[0]));
    return mag.d;
}
 ...
 * --------------------------------------------------------------------------*/
extern double maghead(const magopt_t *opt, const mag_t *data, const double *Cbn, const double *pos)
{
...
static void jacob_head2att(const double *Cbe, const double *pos, double *dhdatt)
{
    ...
    dhdatt[0] = (Cbn[0] * (Cne[4] * Cbe[0] - Cne[3] * Cbe[1]) - Cbn[1] * (Cne[1] * Cbe[0] - Cne[0] * Cbe[1])) /
                (SQR(Cbn[0]) + SQR(Cbn[1]));
}
 ...
 * --------------------------------------------------------------------------*/
extern int magnetometer(insstate_t *ins, const insopt_t *opt, const mag_t *data)
{
    trace(3, "maghead:\n");
