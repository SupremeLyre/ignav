...
{
    const double uraval[] = {0.08, 0.11, 0.15, 0.21, 0.30, 0.43, 0.60, 0.85,
                             1.20, 1.70, 2.40, 3.40, 4.85, 6.85, 9.65, 9.65};
    if (ura < 0 || 15 < ura)
        ura = 15;
    return uraval[ura];
}
...
static int decode_lextof(const unsigned char *buff, int i, gtime_t *tof, gtime_t *toe)
{
    double tt, toes;
    int tow, week;
    char s1[64], s2[64];
...
static int decode_lexhealth(const unsigned char *buff, int i, gtime_t tof, nav_t *nav)
{
    int j, sat;
    ...
    for (j = 0; j < 35; j++)
    {
        ...
        if (j < 3)
            sat = satno(SYS_QZS, j + 193);
        else
            sat = satno(SYS_GPS, j - 2);
        if (!sat)
            continue;
...
static int decode_lexeph(const unsigned char *buff, int i, gtime_t toe, nav_t *nav)
{
    lexeph_t eph = {{0}};
    gtime_t tof;
    unsigned char health;
    ...
    trace(4, "sat=%2d toe=%s pos=%.3f %.3f %.3f vel=%.5f %.5f %.5f\n", sat, time_str(toe, 0), eph.pos[0], eph.pos[1],
          eph.pos[2], eph.vel[0], eph.vel[1], eph.vel[2]);
    trace(4, "clk=%11.3f %8.5f tgd=%7.3f\n", eph.af0 * 1E9, eph.af1 * 1E9, eph.tgd * 1E9);
    trace(4, "isc=%6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f\n", eph.isc[0] * 1E9, eph.isc[1] * 1E9, eph.isc[2] * 1E9,
          eph.isc[3] * 1E9, eph.isc[4] * 1E9, eph.isc[5] * 1E9, eph.isc[6] * 1E9);
    return i;
}
/* decode ionosphere correction field (ref [1] 5.7.2.2.1.3) ------------------*/
static int decode_lexion(const unsigned char *buff, int i, gtime_t tof, nav_t *nav)
{
    lexion_t ion = {{0}};
    int tow, week;
    ...
    trace(4, "t0=%s tspan=%.0f pos0=%.1f %.1f coef=%.3f %.3f %.3f %.3f %.3f %.3f\n", time_str(ion.t0, 0), ion.tspan,
          ion.pos0[0] * R2D, ion.pos0[1] * R2D, ion.coef[0][0], ion.coef[1][0], ion.coef[2][0], ion.coef[0][1],
...
static int lex2rtcm(const unsigned char *msg, int i, unsigned char *buff)
{
    ...
    if (i + 12 >= LEXFRMLEN - LEXHDRLEN - LEXRSLEN)
        return 0;
    ...
    default:
        if (type)
            trace(2, "lex 12: unsupported type=%4d\n", type);
        return 0;
    }
    ...
    *tof = gpst2time(week, tow);
    ...
    while ((n = lex2rtcm(msg->msg, i, buff)))
    {
        ...
        for (j = 0; j < n + 6; j++)
        {
            ...
            if (input_rtcm3(&rtcm, buff[j]) == -1)
                continue;

            /* update ssr corrections in nav data */
            for (k = 0; k < MAXSAT; k++)
            {
                if (!rtcm.ssr[k].update)
                    continue;
...
extern int lexreadmsg(const char *file, int sel, lex_t *lex)
{
    ...
    unsigned int b;
    char buff[1024], *p, file_[1024];
    FILE *fp;

    trace(3, "readmsgs: file=%s sel=%d\n", file, sel);

    strcpy(file_, file);
    if (!(p = strrchr(file_, '.')) || (strcmp(p, ".lex") && strcmp(p, ".LEX")))
        return 0;
    ...
    while (fgets(buff, sizeof(buff), fp))
    {
        ...
        {
            if (sscanf(p, "%2X", &b) == 1)
                lex->msgs[lex->n].msg[i] = (unsigned char)b;
        }
        lex->n++;
    }
    fclose(fp);
 ...
 *-----------------------------------------------------------------------------*/
extern int lexeph2pos(gtime_t time, int sat, const nav_t *nav, double *rs, double *dts, double *var)
{
    const lexeph_t *eph;
    double t, t2, t3;
    int i;
 ...
 *          corrections to navigation data
 *-----------------------------------------------------------------------------*/
extern int lexioncorr(gtime_t time, const nav_t *nav, const double *pos, const double *azel, double *delay, double *var)
{
