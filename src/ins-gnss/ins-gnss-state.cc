...
extern int xnP(const insopt_t *opt)
{
    return 3;
}
/* get number of velocity states---------------------------------------------*/
extern int xnV(const insopt_t *opt)
{
    return 3;
}
/* get number of attitude states---------------------------------------------*/
extern int xnA(const insopt_t *opt)
{
    return 3;
}
/* get number of accl/gyro bias states---------------------------------------*/
extern int xnBa(const insopt_t *opt)
{
    return ((opt)->baopt == INS_BAEST ? 3 : 0);
}
extern int xnBg(const insopt_t *opt)
{
    return ((opt)->bgopt == INS_BGEST ? 3 : 0);
}
/* get number of time synchronization error states---------------------------*/
extern int xnDt(const insopt_t *opt)
{
    return ((opt)->estdt == INS_DTEST ? 1 : 0);
}

/* get number of residual scale factors of gyroscopes------------------------*/
extern int xnSg(const insopt_t *opt)
{
    return ((opt)->estsg == INS_SGEST ? 3 : 0);
}
/* get number of residual scale factors of accl------------------------------*/
extern int xnSa(const insopt_t *opt)
{
    return ((opt)->estsa == INS_SAEST ? 3 : 0);
}

/* get number of non-orthogonal between sensor axes for gyro/accl------------*/
extern int xnRg(const insopt_t *opt)
{
    return ((opt)->estrg == INS_RGEST ? 6 : 0);
}
extern int xnRa(const insopt_t *opt)
{
    return ((opt)->estra == INS_RAEST ? 6 : 0);
}

/* get number of lever arm for body to ant.----------------------------------*/
extern int xnLa(const insopt_t *opt)
{
    return ((opt)->estlever == INS_RAEST ? 3 : 0);
}

/* get number of odometry scale factor---------------------------------------*/
extern int xnOs(const insopt_t *opt)
{
    return ((opt)->estodos ? 1 : 0);
}
/* get number of odometry lever arm from imu body frame----------------------*/
extern int xnOl(const insopt_t *opt)
{
    return ((opt)->estodol ? 3 : 0);
}
/* get number of odometry misalignment of body frame and rear frame----------*/
extern int xnOa(const insopt_t *opt)
{
    return ((opt)->estodoa ? 3 : 0);
}
/* get number of misalignment estimate states of camera to imu body----------*/
extern int xnCm(const insopt_t *opt)
{
    return (opt)->gopt ? ((prcopt_t *)(opt)->gopt)->mode == PMODE_INS_LVO && opt->voopt.estmac ? 3 : 0 : 0;
}
...
extern int xnCl(const insopt_t *opt)
{
    return xnP(opt) + xnV(opt) + xnA(opt) + xnBa(opt) + xnBg(opt) + xnDt(opt) + xnSg(opt) + xnSa(opt) + xnRg(opt) +
           xnRa(opt) + xnLa(opt) + xnOs(opt) + xnOl(opt) + xnOa(opt) + xnCm(opt) + xnVm(opt);
}
/* get number of observation data frequency----------------------------------*/
extern int xnIF(const insopt_t *opt)
{
    return ((opt)->gopt ? ((prcopt_t *)(opt)->gopt)->ionoopt == IONOOPT_IFLC ? 1 : ((prcopt_t *)opt->gopt)->nf : 0);
}
/* get number of receiver clock bias (non-close-loop correction states)------*/
extern int xnRc(const insopt_t *opt)
{
    return ((opt)->gopt ? ((prcopt_t *)(opt)->gopt)->mode == PMODE_INS_TGNSS && (opt)->tc <= INSTC_RTK ? 4 : 0 : 0);
}
...
{
    return ((opt)->gopt ? ((prcopt_t *)(opt)->gopt)->tropopt < TROPOPT_EST
                              ? 0
                              : (((prcopt_t *)(opt)->gopt)->tropopt < TROPOPT_ESTG ? 2 : 6)
                        : 0);
}
...
extern int xnB(const insopt_t *opt)
{
    return ((opt)->gopt ? ((prcopt_t *)(opt)->gopt)->mode == PMODE_INS_TGNSS &&
                                  ((opt)->tc == INSTC_PPK || (opt)->tc == INSTC_RTK)
                              ? (MAXSAT * xnIF(opt))
                              : 0
                        : 0);
}
...
extern int xnX(const insopt_t *opt)
{
    return xnRx(opt) + xnB(opt);
}

extern int xiA(const insopt_t *opt)
{
    return 0;
} /* attitude states index */
extern int xiV(const insopt_t *opt)
{
    return 3;
} /* velocity states index */
extern int xiP(const insopt_t *opt)
{
    return 6;
} /* position states index */
extern int xiBa(const insopt_t *opt)
{
    return 9;
} /* accl bias state index */

/* get index of gyro bias states---------------------------------------------*/
extern int xiBg(const insopt_t *opt)
{
...
{
    return xnA(opt) + xnV(opt) + xnP(opt) + xnBa(opt) + xnBg(opt) + xnDt(opt) + xnSg(opt) + xnSa(opt) + xnRg(opt) +
           xnRa(opt) + xnLa(opt) + xnOs(opt) + xnOl(opt) + xnOa(opt) + xnCm(opt) + xnVm(opt) + xnRc(opt);
}
