...
{
    return (data->accl[0] < MIN_ACCL) && (data->accl[1] < MIN_ACCL) && (fabs(data->accl[2] + gn) < MIN_ACCL) &&
 ...
 * ---------------------------------------------------------------------------*/
extern int coarse_align(insstate_t *ins, const imud_t *data, int n, const insopt_t *opt)
{
    int i, j, k;
    ...
    A[0] = -tan(pos[0]) / g;
    A[2] = -1.0 / g;
    A[3] = 1.0 / (OMGE * cos(pos[0]));
    A[7] = -1.0 / (g * OMGE * cos(pos[0]));

    cross3(fbc, omgc, gn);
    for (i = 0; i < 3; i++)
        B[0 + i * 3] = fbc[i], B[1 + i * 3] = omgc[i];
    for (i = 0; i < 3; i++)
        B[2 + i * 3] = gn[i];
 ...
 * return  : none
 * ----------------------------------------------------------------------*/
extern void asi_blk_mat(double *A, int m, int n, const double *B, int p, int q, int isr, int isc)
{
    int i, j;
    for (i = isr; i < isr + p; i++)
    {
...
static void imu_con_inc(const imud_t *data, int n, const double dt, double *phim, double *dvbm)
{
    int i, j;
    for (i = 0; i < 3; i++)
        phim[i] = dvbm[i] = 0.0;
    for (i = 0; i < n; i++)
        for (j = 0; j < 3; j++)
        {
            phim[j] += data[i].gyro[j] * dt;
            dvbm[j] += data[i].accl[j] * dt;
        }
}
/* initial align fn method-------------------------------------------------*/
static void afnkfinit(const double dt, const double *pos, const double *phi0, const insopt_t *opt, double *phi,
                      double *Q, double *R, double *P0, double *H, double *x0)
{
    ...
    for (i = 0; i < 3; i++)
        Q[i + i * 5] = SQR(pa->web[i]) * dt;
    for (i = 0; i < 2; i++)
        R[i + i * 2] = SQR(pa->wdb[i] / SQRT(dt));
    for (i = 0; i < 3; i++)
        P0[i + i * 5] = SQR(phi0[i]);
    for (i = 3; i < 5; i++)
        P0[i + i * 5] = SQR(pa->eb[i - 3]);
    for (i = 0; i < 5; i++)
        x0[i] = 1E-20;
...
/* align fiter--------------------------------------------------------------*/
static int afnfilter(double *x, double *P, const double *Q, const double *R, const double *H, const double *fn,
                     const double *Phi, int nx, int nv)
{
    int i;
 ...
 * return : 1 :ok, 0:fail
 * -------------------------------------------------------------------------*/
static int alignfn(insstate_t *ins, const imud_t *data, int n, const double *phi0, const insopt_t *opt, const double dt,
                   double *rpyo, quat_t *qo)
{
    int i, j, k, ns;
    ...
    for (k = 0, i = 0; i < n; i += ns)
    {
        ...
        /* filter updates */
        if (!afnfilter(x, P, Q, R, H, fn, Phi, 5, 2))
            continue;
...
static void avnkfinit(const double dt, const double *pos, const double *phi0, const insopt_t *opt, const double *wvn,
                      double *Phi, double *Q, double *R, double *P0, double *H, double *x0)
{
    ...
    for (i = 0; i < 3; i++)
        Q[i + i * 12] = SQR(pa->web[i]) * ddt;
    for (i = 3; i < 6; i++)
        Q[i + i * 12] = SQR(pa->wdb[i - 3]) * ddt;

    for (i = 0; i < 3; i++)
        R[i + i * 3] = SQR(wvn[i]);

    for (i = 0; i < 3; i++)
        P0[i + i * 12] = SQR(phi0[i]);
    for (i = 3; i < 6; i++)
        P0[i + i * 12] = VAR_VEL;
    for (i = 6; i < 9; i++)
        P0[i + i * 12] = SQR(pa->eb[i - 6]);
    for (i = 9; i < 12; i++)
    {
...
static int alignvn(insstate_t *ins, const imud_t *data, int n, const double *phi0, const double *wvn,
                   const insopt_t *opt, const double dt, quat_t *qo, double *rpyo)
{
    int i, j, k, ns;
...
static int arequatclose(const quat_t *q1, const quat_t *q2)
{
    ...
    quat_inv(q1, &qinv);
    quat_mul(&dq, &qinv, q2);
    if ((fabs(1.0 - dq.w) < EPS) && (fabs(norm(dq.vec + 1, 3)) < EPS))
        return 1;
    return 0;
}
...
extern int fine_align(insstate_t *ins, const imud_t *data, int n, const insopt_t *opt)
{
    ...
    if (k1 && k2)
    {
        if (opt->align_fn && opt->align_vn)
        {
            ...
            quat2dcm(&qvn, ins->Cbn);
        else if (opt->align_fn)
            quat2dcm(&qfn, ins->Cbn);
        else
        {
 ...
 * ---------------------------------------------------------------------*/
extern int alignfnex(const double *imu, const double *q0, const double *pos, const double *phi0,
                     const ins_align_t *pasi, double dt, int n, double *att0, double *qo)
{
    ...
    for (i = 0; i < 3; i++)
        Q[i + i * 5] = SQR(pasi->web[i]) * ndt;
    for (i = 0; i < 2; i++)
        R[i + i * 2] = SQR(pasi->wdb[i] / SQRT(ndt));
    for (i = 0; i < 3; i++)
        P[i + i * 5] = SQR(phi0[i]);
    for (i = 3; i < 5; i++)
        P[i + i * 5] = SQR(pasi->eb[i - 3]);
    for (i = 0; i < 5; i++)
        x[i] = 1E-15;
    ...
    for (k = 0, i = 0; i < n; i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            data.gyro[j] = imu[7 * i + j];
            data.accl[j] = imu[7 * i + 3 + j];
        }
        /* check imu data whether is static */
        if (!chkstatic(&data, NULL, -gn[2]))
            continue;
        ...
        for (j = 0; j < 3; j++)
            tv[j] = dvbm[j] / ndt;
        qmulve(q, tv, tt);
        matmul("NN", 3, 1, 3, 1.0, Cnn, tt, 0.0, fn);
 ...
 * note : this function is implemented in enu-rfu-frame
 * ------------------------------------------------------------------------*/
extern int alignvnex(const double *imu, int n, double dt, const double *q0, const double *pos, const double *phi0,
                     const double *wvn, const ins_align_t *pas, double *att0, double *qo)
{
    ...
    for (i = 3; i < 6; i++)
        P[i + i * 12] = VAR_VEL;
    for (i = 6; i < 9; i++)
        P[i + i * 12] = SQR(pas->eb[i - 6]);
    for (i = 9; i < 12; i++)
        P[i + i * 12] = SQR(pas->db[i - 9]);

    for (i = 0; i < 3; i++)
        R[i + i * 3] = SQR(wvn[i]);
    for (i = 0; i < 3; i++)
        Q[i + i * 12] = SQR(pas->web[i]) * dt;
    for (i = 3; i < 6; i++)
    {
 ...
 * ---------------------------------------------------------------------------*/
extern int fine_alignex(insstate_t *ins, const imud_t *data, int n, const insopt_t *opt)
{
    int i, k1 = 1, k2 = 1;
    ...
    if (opt->align_vn)
    {
        k2 = alignvnex(imu, n, pas->dt, q0, ins->rn, pas->phi0, pas->wvn, pas, avn, qvn);
    }
    /* check fn and vn method whether is coincidence */
    if (k1 && k2)
    {
        if (opt->align_fn && opt->align_vn)
        {
            ...
            if (fabs(qfn[0] - qvn[0]) < EPS && fabs(qfn[1] - qvn[1]) < EPS && fabs(qfn[2] - qvn[2]) < EPS &&
                fabs(qfn[3] - qvn[3]) < EPS)
            {

                /* average quaternions, now is a simple way*/
                for (i = 0; i < 4; i++)
                    qavg[i] = (qfn[i] + qvn[i]) / 2.0;
                q2mat(qavg, Ct);
...
static void jacobian_lym(double *x, const double *wnie, const double *fn, double ts, double *F)
{
    int i;
    ...
    jF[3] = -saz * fnz;
    jF[8] = -caz * fnz;
    jF[13] = saz * fnx + caz * fny - (-ay * saz + ax * caz) * fnz;
    jF[4] = caz * fnz;
    jF[9] = -saz * fnz;
    jF[14] = -caz * fnx + saz * fny - (ay * caz + ax * saz) * fnz;
 ...
 * ----------------------------------------------------------------------------*/
extern int fine_align_lym(insstate_t *ins, const imud_t *data, int n, const insopt_t *opt)
{
    int i, j, info = 1;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        /* velocity updating */
        q2mat(q, Cbn);
        matmul33("NNN", Cnn, Cbn, dvbm, 3, 3, 3, 3, dvn);
        for (j = 0; j < 3; j++)
            vn[j] += (dvn[j] - gn[j] * dt);
