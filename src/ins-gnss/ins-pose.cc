...
extern void correctatt(const double *dphi, const double *C, double *Cc)
{
    ...
    static const double I1[3] = {1, 0, 0}, I2[3] = {0, 1, 0}, I3[3] = {0, 0, 1};
...
static void updatecampose(const insopt_t *opt, const pose_meas_t *data, insstate_t *ins)
{
    double C[9] = {0};
...
static void updatedualant(const insopt_t *opt, const pose_meas_t *data, insstate_t *ins)
{
...
static void interp_ins_pose(const insstate_t *ins, double dt, double *Cbe)
{
    ...
    tt = timediff(ins->time, ins->ptime);
    t = (dt + tt) / tt;
    if (fabs(tt) < 1E-6)
    {
...
static void jac_dphi_datt(const double *phi, const double *Cbe, const double *Cne, double *dphidatt)
{
    double Jl[9];
    so3_jac(phi, NULL, Jl);
...
static void jac_dphi_dvma(const double *phi, const double *Cbe, const double *Cne, double *dphidvma)
{
    double Jl[9], T[9];
    so3_jac(phi, NULL, Jl);
...
static int chkest_state(const double *x, const insopt_t *opt)
{
    ...
    if (nbg && x[ibg] == DISFLAG && norm(x + ibg, 3) > 15.0 * D2R)
        flag |= 1;
    if (nvm && x[ivm] == DISFLAG && norm(x + ivm, 3) > 15.0 * D2R)
        flag |= 1;
    if (flag)
    {
...
static int poseantfilt(const insopt_t *opt, const pose_meas_t *data, insstate_t *ins, double dt)
{
    ...
    for (i = 0, nv = 0; i < 3; i++)
    {
        ...
        {
            for (j = ia; j < ia + na; j++)
                H[j + nv * nx] = dphidatt[i + (j - ia) * 3];
            for (j = im; j < im + nm; j++)
                H[j + nv * nx] = dphidvma[i + (j - im) * 3];
        }
        ind[nv] = i;
        nv++;
    }
...
static int posecamfilt(const insopt_t *opt, const pose_meas_t *data, insstate_t *ins, double dt)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        {
            for (j = ia; j < ia + na; j++)
                H[j + nv * nx] = -dphida[i + (j - ia) * 3];
            for (j = im; j < im + nm; j++)
                H[j + nv * nx] = -dphidma[i + (j - im) * 3];
        }
...
extern int posefusion(const insopt_t *opt, const pose_meas_t *data, insstate_t *ins, int flag)
{
    ...
    if (flag == INSUPD_TIME)
    {
        ...
        {
        case POSE_CAM:
            updatecampose(opt, data, ins);
            break;
        case POSE_DUAL_ANT:
            updatedualant(opt, data, ins);
            break;
        }
    }
    else
