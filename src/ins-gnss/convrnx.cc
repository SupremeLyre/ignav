...
typedef struct halfd_tag
{                           /* half-cycle ambiguity data type */
...
typedef struct
{ /* half-cycle ambiguity status type */
...
{
    if (ver >= 2.12 && (sys == SYS_GPS || sys == SYS_QZS || sys == SYS_SBS) && !strcmp(type + 1, "1C"))
    { /* L1C/A */
        strcpy(type + 1, "A");
    }
    else if (ver >= 2.12 && (sys == SYS_GPS || sys == SYS_QZS) &&
             (!strcmp(type + 1, "1S") || !strcmp(type + 1, "1L") || !strcmp(type + 1, "1X")))
    { /* L1C */
        strcpy(type + 1, "B");
    }
    else if (ver >= 2.12 && (sys == SYS_GPS || sys == SYS_QZS) &&
             (!strcmp(type + 1, "2S") || !strcmp(type + 1, "2L") || !strcmp(type + 1, "2X")))
    { /* L2C */
        strcpy(type + 1, "C");
    }
    else if (ver >= 2.12 && sys == SYS_GLO && !strcmp(type + 1, "1C"))
    { /* L1C/A */
        strcpy(type + 1, "A");
    }
    else if (ver >= 2.12 && sys == SYS_GLO && !strcmp(type + 1, "2C"))
    { /* L2C/A */
        strcpy(type + 1, "D");
    }
    else if (sys == SYS_CMP && (!strcmp(type + 1, "1I") || !strcmp(type + 1, "1Q") || !strcmp(type + 1, "1X")))
    { /* B1 */
        strcpy(type + 1, "2");
    }
    else if (!strcmp(type, "C1P") || !strcmp(type, "C1W") || !strcmp(type, "C1Y") || !strcmp(type, "C1N"))
    { /* L1P,P(Y) */
        strcpy(type, "P1");
    }
...
static void rtcm2opt(const rtcm_t *rtcm, const stas_t *stas, rnxopt_t *opt)
{
    ...
    for (p = stas; p; p = p->next)
    {
        sta = &p->sta;
        staid = p->staid;
        if (timediff(p->time, opt->tstart) < DTTOL)
            break;
    }
    if (!sta)
    {
    ...
    {
        if (!*opt->marker)
            sprintf(opt->marker, "%04d", staid);
        sprintf(opt->comment[1] + strlen(opt->comment[1]), ", station ID: %d", staid);
    }
    /* receiver and antenna info */
    if (!*opt->rec[0] && !*opt->rec[1] && !*opt->rec[2])
    {
...
static void setopt_obstype(const unsigned char *codes, const unsigned char *types, int sys, rnxopt_t *opt)
{
    ...
    for (i = 0; codes[i]; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            else if (opt->nobs[sys] < MAXOBSTYPE)
            { /* ver.3 */
...
static void update_stas(stas_t **stas, strfile_t *str)
{
    ...
    p->next = *stas;
    *stas = p;
    trace(2, "update_stas: staid=%d time=%s\n", str->rtcm.staid, time_str(str->rtcm.time, 0));
}
...
static void update_halfc(halfc_t *halfc, obsd_t *obs)
{
    ...
    for (i = 0; i < NFREQ + NEXOBS; i++)
    {
        ...
        if (obs->LLI[i] & LLI_HALFC)
        {
            ...
            {
                if (!(p = (halfd_t *)calloc(sizeof(halfd_t), 1)))
                    return;
                p->ts = p->te = obs->time;
        ...
        else if (halfc->stat[obs->sat - 1][i] && obs->L[i] != 0.0 && (p = halfc->data[obs->sat - 1][i]))
        {
...
static void resolve_halfc(halfc_t *halfc, obsd_t *obs)
{
    ...
    for (i = 0; i < NFREQ + NEXOBS; i++)
    {
        for (p = halfc->data[obs->sat - 1][i]; p; p = p->next)
        {
            if (!p->stat)
                continue;
            if (timediff(obs->time, p->ts) < -DTTOL || timediff(obs->time, p->te) > DTTOL)
                continue;
...
static int scan_obstype(int format, char **files, int nf, rnxopt_t *opt, stas_t **stas, halfc_t *halfc, gtime_t *time)
{
    strfile_t *str;
    ...
    for (m = 0; m < nf && !abort; m++)
    {
        ...
        while ((type = input_strfile(str)) >= -1)
        {
            ...
            if (opt->te.time && timediff(str->obs->data[0].time, opt->te) > 10.0)
                break;
...
static void set_obstype(int format, rnxopt_t *opt)
{
    ...
    static const unsigned char codes_rtcm2[NSATSYS][8] = {
        /* rtcm2 */
        {CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P}, {CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P}, {0}, {0}, {0}, {0}, {0}};
    ...
    static const unsigned char codes_oem4[NSATSYS][8] = {/* novatel oem6 */
                                                         ...
                                                         {CODE_L1I, CODE_L7I},
                                                         {0}};
    static const unsigned char codes_cres[NSATSYS][8] = {
        /* hemisphere */
        {CODE_L1C, CODE_L2P}, {CODE_L1C, CODE_L2P}, {0}, {0}, {CODE_L1C}, {0}, {0}};
    static const unsigned char codes_javad[NSATSYS][8] = {/* javad */
                                                          ...
                                                          {CODE_L1I, CODE_L7I},
                                                          {0}};
    static const unsigned char codes_rinex[NSATSYS][32] = {
        /* rinex and binex */
        {CODE_L1C, CODE_L1P, CODE_L1W, CODE_L1Y, CODE_L1M, CODE_L1N, CODE_L1S, CODE_L1L, CODE_L2C, CODE_L2D, CODE_L2S,
         CODE_L2L, CODE_L2X, CODE_L2P, CODE_L2W, CODE_L2Y, CODE_L2M, CODE_L2N, CODE_L5I, CODE_L5Q, CODE_L5X},
        {CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P, CODE_L3I, CODE_L3Q, CODE_L3X},
        {CODE_L1C, CODE_L1A, CODE_L1B, CODE_L1X, CODE_L1Z, CODE_L5I, CODE_L5Q, CODE_L5X, CODE_L6A, CODE_L6B, CODE_L6C,
         CODE_L6X, CODE_L6Z, CODE_L7I, CODE_L7Q, CODE_L7X, CODE_L8I, CODE_L8Q, CODE_L8X},
        {CODE_L1C, CODE_L1S, CODE_L1L, CODE_L1X, CODE_L1Z, CODE_L2S, CODE_L2L, CODE_L2X, CODE_L5I, CODE_L5Q, CODE_L5X,
         CODE_L6S, CODE_L6L, CODE_L6X},
        {CODE_L1C, CODE_L5I, CODE_L5Q, CODE_L5X},
        {CODE_L1I, CODE_L1Q, CODE_L1X, CODE_L7I, CODE_L7Q, CODE_L7X, CODE_L6I, CODE_L6Q, CODE_L6X},
        {CODE_L5A, CODE_L5B, CODE_L5C, CODE_L5X, CODE_L9A, CODE_L9B, CODE_L9C, CODE_L9X}};
    static const unsigned char codes_rt17[NSATSYS][8] = {
        /* rt17 */
        {CODE_L1C, CODE_L1P, CODE_L2C, CODE_L2P, CODE_L2W}, {0}, {0}, {0}, {0}, {0}, {0},
    };
    static const unsigned char codes_cmr[NSATSYS][8] = {
        /* cmr */
    ...
    static const unsigned char codes_other[NSATSYS][8] = {/* others */
                                                          {CODE_L1C}, {CODE_L1C}, {CODE_L1C}, {CODE_L1C},
                                                          {CODE_L1C}, {CODE_L1I}, {0}};
...
static void restslips(unsigned char slips[][NFREQ + NEXOBS], obsd_t *data, int n)
{
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < NFREQ + NEXOBS; j++)
        {
            if (slips[data[i].sat - 1][j] & 1)
                data[i].LLI[j] |= 1;
            slips[data[i].sat - 1][j] = 0;
        }
}
/* open output files ---------------------------------------------------------*/
static int openfile(FILE **ofp, char *files[], const char *file, const rnxopt_t *opt, nav_t *nav)
{
    char path[1024];
    int i;
...
static void closefile(FILE **ofp, const rnxopt_t *opt, nav_t *nav)
{
    ...
    for (i = 0; i < NOUTFILE; i++)
    {
        ...
        case 5:
            ...
            break;
        case 6:
            outrnxlnavh(ofp[6], opt, nav);
            break;
        case 7:
            outrnxlnavh(ofp[7], opt, nav);
            break;
        }
        fclose(ofp[i]);
    }
...
static void outrnxevent(FILE *fp, rnxopt_t *opt, int staid, stas_t *stas)
{
    ...
    {
        if (!p->sta.deltype && norm(p->sta.del, 3) > 0.0)
        {                           /* enu */
            del[0] = p->sta.del[2]; /* h */
            del[1] = p->sta.del[0]; /* e */
            del[2] = p->sta.del[1]; /* n */
        }
        else if (norm(p->sta.pos, 3) > 0.0)
        { /* xyz */
...
/* convert obs message -------------------------------------------------------*/
static void convobs(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *staid, stas_t *stas, halfc_t *halfc, int *n,
                    unsigned char slips[][NFREQ + NEXOBS])
{
    ...
    if (opt->ts.time != 0 && timediff(time, opt->ts) < -opt->ttol)
        return;
    ...
    if ((str->format == STRFMT_RTCM2 || str->format == STRFMT_RTCM3) && str->rtcm.staid != *staid)
    {
        if (*staid >= 0)
        {
...
static void convsbs(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n)
{
    ...
    if (ofp[NOUTFILE - 1])
    { /* output sbas log */
        if (screent(gpst2time(str->raw.sbsmsg.week, str->raw.sbsmsg.tow), opt->ts, opt->te, 0.0))
        {
            sbsoutmsg(ofp[NOUTFILE - 1], &str->raw.sbsmsg);
            n[NOUTFILE - 1]++;
        }
    }
    if (!(opt->navsys & SYS_SBS) || msg != 9 || !screent(str->time, ts1, te1, 0.0))
        return;
...
static void convlex(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n)
{
    ...
    {
        lexoutmsg(ofp[NOUTFILE - 1], &str->raw.lexmsg);
        n[NOUTFILE - 1]++;
    }
}
/* set approx position -------------------------------------------------------*/
static void setapppos(strfile_t *str, rnxopt_t *opt)
{
    ...
    if (!pntpos(str->obs->data, str->obs->n, str->nav, &prcopt, &sol, NULL, NULL, NULL, msg))
    {
...
static int convrnx_s(int sess, int format, rnxopt_t *opt, const char *file, char **ofile)
{
    ...
    if (opt->scanobs)
    {
        ...
        /* scan observation types and station parameters */
        if (!scan_obstype(format, epath, nf, opt, &stas, &halfc, &time))
            return 0;
    }
    else
    {
    ...
    for (i = 0; i < nf && !abort; i++)
    {
        ...
        for (j = 0; (type = input_strfile(str)) >= -1; j++)
        {
            ...
            {
            case 1:
                convobs(ofp, opt, str, &staid, stas, &halfc, n, slips);
                break;
            case 2:
...
extern int convrnx(int format, rnxopt_t *opt, const char *file, char **ofile)
{
    ...
    else if (timediff(opt->ts, opt->te) <= 0.0)
    {
        ...
        { /* for each session */
            ...
            if (timediff(opt_.ts, opt->te) > 0.0)
                break;
