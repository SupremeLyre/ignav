...
static int checkpri(const char *opt, int sys, int code, int freq)
{
    ...
    if (sys == SYS_GPS)
    {
        if (strstr(opt, "-GL1W") && freq == 0)
            return code == CODE_L1W ? 0 : -1;
        if (strstr(opt, "-GL1X") && freq == 0)
    ...
    {
        if (strstr(opt, "-RL1C") && freq == 0)
            return code == CODE_L1C ? 0 : -1;
    ...
    {
        if (strstr(opt, "-JL1Z") && freq == 0)
            return code == CODE_L1Z ? 0 : -1;
        if (strstr(opt, "-JL1X") && freq == 0)
            return code == CODE_L1X ? 0 : -1;
        if (code == CODE_L1Z)
            return nex < 1 ? -1 : NFREQ;
        if (code == CODE_L1X)
...
static double freq_sys(int sys, int freq, int freqn)
{
    if (sys == SYS_GLO)
    {
        ...
        {
        case 0:
            return FREQ1_GLO + DFRQ1_GLO * freqn; /* L1 */
        case 1:
            return FREQ2_GLO + DFRQ2_GLO * freqn; /* L2 */
        case 2:
            return FREQ3_GLO; /* L3 */
        }
        return 0.0;
...
static int decode_eph(raw_t *raw, int sys)
{
    ...
    {
        if (timediff(raw->nav.eph[eph.sat - 1].toe, eph.toe) == 0.0 && raw->nav.eph[eph.sat - 1].iode == eph.iode &&
...
static int decode_NE(raw_t *raw)
{
    ...
    if (raw->nav.geph[prn - MINPRNGLO].toe.time && geph.frq != raw->nav.geph[prn - MINPRNGLO].frq)
    {
        trace(2, "javad NE illegal freq change: prn=%2d frq=%2d->%2d\n", prn, raw->nav.geph[prn - MINPRNGLO].frq,
              geph.frq);
        return -1;
    }
    if (!strstr(raw->opt, "-EPHALL"))
...
static int decode_Rx(raw_t *raw, char code)
{
    ...
    if (!is_meas(code) || raw->tod < 0 || raw->obuf.n == 0)
        return 0;
    ...
    if (raw->nbyte == 0)
    {
        for (i = 0;; i++)
        {
            if ((data = fgetc(fp)) == EOF)
                return endfile(raw);
            if (sync_javad(raw->buff, (unsigned char)data))
                break;
            if (i >= 4096)
                return 0;
        }
