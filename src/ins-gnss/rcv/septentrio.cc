    ...
    for (i = 0; i < nsat && i < MAXOBS; i++)
    {
        ...
        if (h <= NFREQ + NEXOBS)
        {
            ...
            if (LockTime != 65535)
            {
                ...
                if (locktime[sat][signType1] > LockTime)
        ...
        for (ii = 0; ii < (int)SB2Num; ii++)
        {
            ...
            offsetMSB = U1(p + 3);
            CodeOffsetMSB =
                ((offsetMSB & 0x04) == 0x04) ? offsetMSB | ~((int32_t)0x03) : offsetMSB & 0x03; /* bit[0-2] */
            DopplerOffsetMSB = ((offsetMSB & 0x80) == 0x80) ? (offsetMSB >> 3) | ~((int32_t)0x1f)
            ...
            if ((DopplerOffsetMSB == -16) && (DopplerOffsetLSB == 0))
                ...
                dopplerType2 = dopplerType1 * alpha + (DopplerOffsetMSB * 65536. + DopplerOffsetLSB) * 1E-4;
            ...
            /* store signal info */
            if ((h <= NFREQ + NEXOBS) && (pri > getcodepri(sys, raw->obs.data[n].code[h], raw->opt)))
            {
...
static int decode_sbasnav(raw_t *raw)
{
    ...
    if (!strstr(raw->opt, "-EPHALL"))
    {
        if (fabs(timediff(eph.t0, raw->nav.seph[prn - 120].t0)) < 1.0 && eph.sva == raw->nav.seph[prn - 120].sva)
            return 0;
    }
...
static int decode_georaw(raw_t *raw)
{
    ...
    if (crc != rtk_crc24q(buff, 29))
        return 0;

    for (i = 0; i < 29; i++)
        raw->sbsmsg.msg[i] = buff[i];
    ...
    if (raw->nbyte == 0)
    {
        for (i = 0;; i++)
        {
            if ((data = fgetc(fp)) == EOF)
                return -2;
            if (sync_sbf(raw->buff, (unsigned char)data))
                break;
            if (i >= MAXRAWLEN)
                return 0;
        }
