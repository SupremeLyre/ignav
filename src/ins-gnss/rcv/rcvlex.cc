...
static int decode_lexraw(raw_t *raw)
{
    ...
    for (i = 0; i < 17 && p + 38 <= raw->buff + raw->len - 4; i++)
    {
        ...
        for (j = 0; j < NFREQ; j++)
        {
            ...
            raw->obs.data[n].code[3] = type == 0x1A ? CODE_L6S : (type == 0x1B ? CODE_L6L : CODE_NONE);
            n++;
        }
    }
    raw->obs.n = n;
...
static int decode_lexr(raw_t *raw)
{
    ...
    trace(3, "decode_lexr: type=%04X len=%3d stat=%08X time=%s\n", type, raw->len, stat, time_str(raw->time, 3));

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "LEXR 0x%04X (%4d): stat=%08X week=%d tow=%10.3f", type, raw->len, stat, week,
                tow / 1000.0);
    }
    switch (type)
    {
...
extern int input_lexrf(raw_t *raw, FILE *fp)
{
    ...
    for (i = 0; i < 4096; i++)
    {
        if ((data = fgetc(fp)) == EOF)
            return -2;
        if ((ret = input_lexr(raw, (unsigned char)data)))
            return ret;
    }
    return 0; /* return at every 4k bytes */
}
/* generate lex receiver raw message ------------------------------------------*/
static int genmsg(unsigned char *buff, unsigned short id, unsigned char *data, unsigned short len)
{
...
extern int gen_lexr(const char *msg, unsigned char *buff)
{
    ...
    else if (!strcmp(args[0], "CMD-CH"))
    {
        ...
        if (narg > 2)
            *q++ = (unsigned char)atoi(args[2]);
        if (narg > 3)
            *q = (unsigned char)atoi(args[3]);
    ...
    else if (!strcmp(args[0], "CMD-TIME"))
    {
        ...
        for (i = 0; i < 6; i++)
            ep[i] = (unsigned short)epoch[i];
        for (i = 1; i >= 0; i--)
            *q++ = *((unsigned char *)&ep[0] + i);
        for (i = 0; i < 5; i++)
            *q++ = (unsigned char)ep[i + 1];
    ...
    else if (!strcmp(args[0], "CMD-POS"))
    {
        for (i = 0; i < 3; i++)
            if (i + 1 < narg)
                pos[i] = atof(args[i + 1]) * (i < 2 ? D2R : 1.0);
        *q++ = 0;
        for (i = 7; i >= 0; i--)
            *q++ = *((unsigned char *)&pos[0] + i);
        for (i = 7; i >= 0; i--)
            *q++ = *((unsigned char *)&pos[1] + i);
        for (i = 7; i >= 0; i--)
            *q++ = *((unsigned char *)&pos[2] + i);
