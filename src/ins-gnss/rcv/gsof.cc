...
static void decode_type01(raw_t *raw, unsigned char *data)
{
    ...
    raw->gsof.ns = data[6];
    if ((data[8] & 0x00) == 0x00)
        raw->gsof.solq = SOLQ_SINGLE;
    if ((data[8] & 0x01) == 0x01)
        raw->gsof.solq = SOLQ_DGPS;
    if ((data[8] & 0x03) == 0x03)
        raw->gsof.solq = SOLQ_FLOAT;
    if ((data[8] & 0x07) == 0x07)
        raw->gsof.solq = SOLQ_FIX;

    sow = getu32(&data) / 1000.0;
    week = getu16(&data);
...
static void decode_gsof(raw_t *raw)
{
    ...
    while (cb < raw->len - GSOFMIDLEN && raw->len > 0)
    {
        ...
        {
        case GSOFTYPE01:
            decode_type01(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE02:
            decode_type02(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE03:
            decode_type03(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE06:
            decode_type06(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE08:
            decode_type08(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE09:
            decode_type09(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE10:
            decode_type10(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE11:
            decode_type11(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE12:
            decode_type12(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE38:
            decode_type38(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        case GSOFTYPE41:
            decode_type41(raw, raw->buff + GSOFMMLEN + nt + cb);
            break;
        default:
...
static int decode_gsoff(raw_t *raw, unsigned char data)
{
    ...
    {
        raw->nbyte = 0;
        return 0; /* no gsof message */
    }
    if (raw->nbyte < raw->len + 6)
        return 0;
...
extern int input_gsof(raw_t *raw, unsigned char data)
{
    ...
    if (raw->dire)
        return decode_gsofb(raw, data);
    else
        return decode_gsoff(raw, data);
}
...
extern int input_gsoff(raw_t *raw, FILE *fp)
{
    ...
    for (i = 0; i < 4096; i++)
    {
        if ((data = fgetc(fp)) == EOF)
            return -2;
        if ((ret = input_gsof(raw, (unsigned char)data)))
            return ret;
    }
...
extern int sortgsof(gsof_data_t *gsof)
{
    ...
    for (i = n = 0; i < gsof->n; i = j, n++)
    {
        ...
        {
            if (timediff(gsof->data[j].t, gsof->data[i].t) > DTTOL)
                break;
        }
    }
    return n;
