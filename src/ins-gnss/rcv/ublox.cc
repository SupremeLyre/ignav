 ...
 *     [5] ublox-AG, UBX-13003221-R09, u-blox 8 /u-blox M8 Receiver Description
 *         including Protocol Specification V15.00-23.00, January, 2017
 *
...
#define ID_NAVATT 0x0105   /* ubx message id: attitude solution info */
#define ID_NAVSOL 0x0106   /* ubx message id: nav solution info */
#define ID_NAVPVT 0x0107   /* ubx message id: pvt solution message */
#define ID_NAVTIME 0x0120  /* ubx message id: nav time gps */
#define ID_NAVCLK 0x0122   /* ubx message id: nav clock solution */
#define ID_NAVGPST 0x0124  /* ubx message id: nav gps time solution */
#define ID_NAVEOE 0x0161   /* ubx message id: end Of epoch */
...
#define ID_ESFINS 0x1015   /* ubx message id: vehicle dynamics information */
#define ID_ESFRAW 0x1003   /* ubx message id: imu raw measurement data */
#define ID_ESFSTA 0x1010   /* ubx message id: external sensor fusion status information */
...
#define IMUFRQ 100    /* imu measurement data frequency */
...
static void le2be(const unsigned char *i, int n, unsigned char *o)
{
    int k;
    for (k = 0; k < n; k++)
        o[k] = i[n - 1 - k];
 ...
 * args   :  unsign char *b  I  sign binary data (big-endian)
 *           int n           I  number of binary data
 * return sign decimal number
 * -------------------------------------------------------------------------*/
extern long bin2dec(const unsigned char *b, int n)
{
    int i, j;
    long ret;
    unsigned long s;

    for (ret = 0, s = 0x01, i = n - 1, j = 0; i >= 0; i--)
    {
        ret += ((b[i] & 0x0F)) * pow(16, j++);
        s = s << 4;
        ret += ((b[i] & 0xF0) >> 4) * pow(16, j++);
        s = s << 4;
    }
    return ((b[0] & (0x01 << 7)) == 0 ? ret : -1 * (s - ret));
}
/* checksum ------------------------------------------------------------------*/
...
static int decode_rxmrawx(raw_t *raw)
{
    ...
    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX RXM-RAWX  (%4d): time=%s nsat=%d", raw->len, time_str(time, 2), U1(p + 11));
    }
    /* time tag adjustment option (-TADJ) */
    if ((q = strstr(raw->opt, "-TADJ=")))
    {
...
static int decode_alm1(int sat, raw_t *raw)
{
    ...
    if (sys == SYS_GPS)
    {
        decode_frame(raw->subfrm[sat - 1] + 90, NULL, raw->nav.alm, raw->nav.ion_gps, raw->nav.utc_gps,
                     &raw->nav.leaps);
        adj_utcweek(raw->time, raw->nav.utc_gps);
    }
    else if (sys == SYS_QZS)
    {
        decode_frame(raw->subfrm[sat - 1] + 90, NULL, raw->nav.alm, raw->nav.ion_qzs, raw->nav.utc_qzs,
                     &raw->nav.leaps);
...
static int decode_alm2(int sat, raw_t *raw)
{
    ...
    {
        decode_frame(raw->subfrm[sat - 1] + 120, NULL, raw->nav.alm, raw->nav.ion_qzs, raw->nav.utc_qzs,
                     &raw->nav.leaps);
        adj_utcweek(raw->time, raw->nav.utc_qzs);
    }
    return 0;
...
/* decode ubx-nav-att: attitude solution--------------------------------------*/
static int decode_navatt(raw_t *raw)
{
    unsigned char *p = raw->buff + 6;

    trace(4, "decode_navatt: len=%d\n", raw->len);

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX NAV-ATT   (%4d):", raw->len);
    }
    raw->sol.att[0] = I4(p + 8) * 1E-5 * D2R;
    ...
    return raw->sol.qa[0] && raw->sol.qa[1] && raw->sol.qa[2];
}
/* decode ubx-nav-clk: clock solution-----------------------------------------*/
static int decode_navclk(raw_t *raw)
{
    unsigned char *p = raw->buff + 6;

    trace(4, "decode_navpvt: len=%d\n", raw->len);

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX NAV-CLK   (%4d):", raw->len);
    }
...
static int decode_navgpst(raw_t *raw)
{
    int itow, ftow, week;
    unsigned char *p = raw->buff + 6;

    trace(4, "decode_navgpst: len=%d\n", raw->len);

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX NAV-GPST   (%4d):", raw->len);
    }
    itow = U4(p);
    ftow = I4(p + 4);
    week = U2(p + 8);

    if ((U1(p + 15) & 0x0C) == 0x0C)
    {
        raw->tflg |= 1;
        raw->time = gpst2time(week, itow * 1E-3 + ftow * 1E-9);
        raw->sol.time = raw->time;
    }
    return 1;
}
/* decode ubx-nav-pvt: navigation position/velocity solution------------------*/
static int decode_navpvt(raw_t *raw)
{
    int i;
    double t[6], llh[3], vn[3], Cne[9], head;
    double sn[9] = {0}, se[9];
    unsigned char *p = raw->buff + 6;
    static gtime_t t0;
    ...
    {
        t0 = epoch2time(t);
        raw->time = utc2gpst(t0);
        raw->tflg |= 1; /* valid time flag */
    }
    llh[0] = I4(p + 28) * 1E-7 * D2R; /* lat/rad */
    llh[1] = I4(p + 24) * 1E-7 * D2R; /* lon/rad */
    llh[2] = I4(p + 32) * 1E-3;       /* h/m */
    pos2ecef(llh, raw->sol.rr);

    vn[0] = I4(p + 48) * 1E-3; /* vn(m/s) */
    vn[1] = I4(p + 52) * 1E-3; /* ve(m/s) */
    vn[2] = I4(p + 56) * 1E-3; /* vd(m/s) */
    ned2xyz(llh, Cne);
    matmul("NN", 3, 1, 3, 1.0, Cne, vn, 0.0, raw->sol.rr + 3);

    head = vel2head(vn);
    sn[0] = SQR(U4(p + 40) * cos(head) * 1E-3); /* sigma of north */
    sn[4] = SQR(U4(p + 40) * sin(head) * 1E-3); /* sigma of east */
    ...
    /* check fix flag from message */
    if ((U1(p + 21) & 0x81) == 0x81)
        raw->sol.stat = SOLQ_FIX;
    else if ((U1(p + 21) & 0x41) == 0x41)
        raw->sol.stat = SOLQ_FLOAT;
    else if ((U1(p + 21) & 0x01) == 0x01)
        raw->sol.stat = SOLQ_DGPS;
    else
    {
        raw->sol.stat = SOLQ_SINGLE;
    }
    return 5;
}
...
/* decode ubx-nav-eoe: end of epoch-------------------------------------------
 * note: ubx-nav-eoe message is necessary if it is ublox NEO-M8 module
 * --------------------------------------------------------------------------*/
static int decode_naveoe(raw_t *raw)
{
    double sow;
    unsigned char *p = raw->buff + 6;

    trace(4, "decode_naveoe: len=%d\n", raw->len);

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX NAV-EOE  (%4d):", raw->len);
    }
    if (!raw->tflg)
        return 0;

    sow = U4(p) * 1E-3;
    if (fabs(sow - time2gpst(raw->time, NULL)) > DTTOL)
    {
        return 0;
    }
    return 0;
}
/* decode ubx-esf-ins: vehicle dynamics information---------------------------*/
static int decode_esfins(raw_t *raw)
{
    trace(4, "decode_esfins: len=%d\n", raw->len);

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX ESF-INS  (%4d):", raw->len);
    }
    return 0;
}
/* decode ubx-esf-status: external sensor fusion status information-----------*/
static int decode_esfsta(raw_t *raw)
{
    trace(4, "decode_esfins: len=%d\n", raw->len);

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX ESF-STATUS  (%4d):", raw->len);
    }
    return 0;
}
/* decode ubx-esf-imu-raw: raw sensor measurements----------------------------*/
static int decode_esfraw(raw_t *raw)
{
    int i;
    u8 f = 0x00;
    unsigned char *p = raw->buff + 6, t[8];
    imud_t imus = {0};

    trace(4, "decode_esfraw: len=%d\n", raw->len);

    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX ESF-RAW  (%4d):", raw->len);
    }
    raw->imut.n = 0;
    for (i = 0; i < (raw->len - 8) / 8; i++)
    {
        ...
        {
        case 0x10:
            imus.accl[0] = bin2dec(t + 1, 3) * P2_10;
            f |= 0x01;
            break;
        case 0x11:
            imus.accl[1] = bin2dec(t + 1, 3) * P2_10;
            f |= 0x02;
            break;
        case 0x12:
            imus.accl[2] = bin2dec(t + 1, 3) * P2_10;
            f |= 0x04;
            break;
        case 0x0E:
            imus.gyro[0] = bin2dec(t + 1, 3) * P2_12;
            f |= 0x08;
            break;
        case 0x0D:
            imus.gyro[1] = bin2dec(t + 1, 3) * P2_12;
            f |= 0x10;
            break;
        case 0x05:
            imus.gyro[2] = bin2dec(t + 1, 3) * P2_12;
            f |= 0x20;
            break;
        case 0x0C:
            imus.temp = bin2dec(t + 1, 3) * 1E-02;
            f |= 0x40;
            break;
        }
        imus.imuc = U4(p + i * 8);
...
static int decode_enav(raw_t *raw, int sat, int off)
{
    ...
    if (!strstr(raw->opt, "-EPHALL"))
    {
        if (eph.iode == raw->nav.eph[sat - 1].iode && /* unchanged */
            timediff(eph.toe, raw->nav.eph[sat - 1].toe) == 0.0 && timediff(eph.toc, raw->nav.eph[sat - 1].toc) == 0.0)
            return 0;
    }
    eph.sat = sat;
    raw->nav.eph[sat - 1] = eph;
...
static int decode_cnav(raw_t *raw, int sat, int off)
{
    ...
    if (!strstr(raw->opt, "-EPHALL"))
    {
        if (timediff(eph.toe, raw->nav.eph[sat - 1].toe) == 0.0 && eph.iode == raw->nav.eph[sat - 1].iode &&
...
/* decode ublox raw message --------------------------------------------------*/
static int decode_ubxm8(raw_t *raw)
{
    int type = (U1(raw->buff + 2) << 8) + U1(raw->buff + 3);
    ...
    {
    case ID_ESFINS:
        return decode_esfins(raw);
    case ID_ESFSTA:
        return decode_esfsta(raw);
    case ID_ESFRAW:
        return decode_esfraw(raw);
    case ID_NAVEOE:
        return decode_naveoe(raw);
    }
    if (raw->outtype)
    {
        sprintf(raw->msgtype, "UBX 0x%02X 0x%02X (%4d)", type >> 8, type & 0xF, raw->len);
...
/* decode ublox raw message --------------------------------------------------*/
static int decode_ubxsol(raw_t *raw)
{
    int type = (U1(raw->buff + 2) << 8) + U1(raw->buff + 3);
    ...
    case ID_NAVATT:
        return decode_navatt(raw);
    case ID_NAVCLK:
        return decode_navclk(raw);
    case ID_NAVGPST:
        return decode_navgpst(raw);
    case ID_NAVPVT:
...
extern int input_ubxm8(raw_t *raw, unsigned char data)
{
    trace(5, "input_ubxm8: data=%02x\n", data);
    ...
    /* decode ublox raw message */
    return decode_ubxm8(raw);
}
extern int input_ubxsol(raw_t *raw, unsigned char data)
{
    trace(5, "input_ubxm8: data=%02x\n", data);
...
extern int input_ubxm8f(raw_t *raw, FILE *fp)
{
    int i, data;
...
extern int input_ubxsolf(raw_t *raw, FILE *fp)
{
    int i, data;
...
{
    const char *cmd[] = {"PRT",   "USB",  "MSG",  "NMEA",   "RATE",      "CFG",  "TP",     "NAV2", "DAT",
                         "INF",   "RST",  "RXM",  "ANT",    "FXN",       "SBAS", "LIC",    "TM",   "TM2",
                         "TMODE", "EKF",  "GNSS", "ITFM",   "LOGFILTER", "NAV5", "NAVX5",  "ODO",  "PM2",
                         "PWR",   "RINV", "SMGR", "TMODE2", "TMODE3",    "TPS",  "TXSLOT", ""};
    const unsigned char id[] = {0x00, 0x1B, 0x01, 0x17, 0x08, 0x09, 0x07, 0x1A, 0x06, 0x02, 0x04, 0x11,
