...
static unsigned char *decode_bnx_7f_05_obs(raw_t *raw, unsigned char *buff, int sat, int nobs, obsd_t *data)
{
    ...
    for (i = 0; i < NFREQ; i++)
    {
        ...
        else
        {
            ...
            {
                wl = CLIGHT / (freq[k] == 1 ? FREQ1_GLO + DFRQ1_GLO * fcn : FREQ2_GLO + DFRQ2_GLO * fcn);
            }
            data->P[i] = range[k];
