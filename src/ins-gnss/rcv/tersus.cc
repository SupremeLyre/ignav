 ...
 *-----------------------------------------------------------------------------*/
static int decode_trackstat(unsigned int stat, int *sys, int *code, int *track, int *plock, int *clock, int *parity,
                            int *halfc)
{
...
static int decode_rangeb(raw_t *raw)
{
    ...
    for (i = 0, p += 4; i < nobs; i++, p += 44)
    {
        ...
        if ((freq = decode_trackstat(U4(p + 40), &sys, &code, &track, &plock, &clock, &parity, &halfc)) < 0)
            continue;
        ...
        if ((index = obsindex(&raw->obs, raw->time, sat)) >= 0)
        {
            ...
            raw->obs.data[index].SNR[pos] = 0.0 <= snr && snr < 255.0 ? (unsigned char)(snr * 4.0 + 0.5) : 0;
...
static int decode_tersus(raw_t *raw)
{
    ...
    if (raw->outtype)
    {
        sprintf(raw->msgtype, "TERSUS%4d (%4d): msg=%d %s", type, raw->len, msg, time_str(gpst2time(week, tow), 2));
    }
    switch (type)
    {
    case ID_RANGE:
    ...
    if (raw->nbyte == 0)
    {
        for (i = 0;; i++)
        {
            if ((data = fgetc(fp)) == EOF)
                return -2;
            if (sync_tersus(raw->buff, (unsigned char)data))
                break;
            if (i >= 4096)
                return 0;
        }
