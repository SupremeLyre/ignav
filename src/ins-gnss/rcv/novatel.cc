...
static int decode_rgeb(raw_t *raw)
{
    ...
    for (i = 0, p += 20; i < nobs; i++, p += 44)
    {
        ...
        if ((index = obsindex(&raw->obs, raw->time, sat)) >= 0)
        {
            ...
            raw->obs.data[index].SNR[freq] = 0.0 <= snr && snr < 255.0 ? (unsigned char)(snr * 4.0 + 0.5) : 0;
...
static int decode_oem4(raw_t *raw)
{
    ...
    if (raw->outtype)
    {
        sprintf(raw->msgtype, "OEM4 %4d (%4d): msg=%d %s", type, raw->len, msg, time_str(gpst2time(week, tow), 2));
    }
    if (msg != 0)
