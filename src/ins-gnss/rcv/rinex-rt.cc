...
/* decode obs header ---------------------------------------------------------*/
static void decode_obsh(char *buff, int *tsys, char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
...
/* set signal index ----------------------------------------------------------*/
static void sigindex(int sys, const char *opt, char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    const char *p;
    char str[8], *optstr = "";
    double shift;
...
extern int input_rinex(raw_t *raw, unsigned char data)
{
    ...
    { /* buffer overflow and reset decoder */
        resetrnx(raw);
        raw->nbyte = 0;
        return 0;
    }
    ...
    else
    {
        ...
        if (!raw->rinex.start && (raw->rinex.nsat = decode_obsepoch(p, &raw->rinex.time, &flag)))
        {
            raw->rinex.start = 1;
            clearbuff(raw);
            return 0;
        ...
        else if (flag <= 2 || flag == 6)
        {
            ...
            /* decode obs data */
            if (decode_obsdata(p, mask, raw->rinex.index, raw->rinex.obs + raw->rinex.n) && raw->rinex.n < MAXOBS)
                raw->rinex.n++;
        }
