...
| See reference #3.
*/
static const unsigned char SnrTable[][2] = {{0, 0},   {30, 4},  {32, 8},  {34, 12}, {36, 16}, {38, 20},
...
extern int input_cmrf(raw_t *Raw, FILE *fp)
{
    ...
    for (i = 0; i < 4096; i++)
    {
        if ((Data = fgetc(fp)) == EOF)
            return -2;
        if ((Ret = input_cmr(Raw, (unsigned char)Data)))
            return Ret;
    }
...
static int DecodeCmrType0(raw_t *Raw)
{
    ...
    for (Slot = 0; (Slot < nsat) && (Slot < MAXOBS); Slot++)
    {
        ...
        Sat = b->Sat - 1;
        if (Cmr->SlipV[Sat][0] && Cmr->SlipC[Sat][0] != b->Slip[0])
            b->LLI[0] |= 1;
        if (Cmr->SlipV[Sat][1] && Cmr->SlipC[Sat][1] != b->Slip[1])
            b->LLI[1] |= 1;
...
static int ReferenceCmrObs(raw_t *Raw, gtime_t Time, unsigned char Type, double P0, obsbd_t *b)
{
    ...
    {
        if ((L1WaveLength = satwavelen(b->Sat, 0, Nav) == 0.0) || (L2WaveLength = satwavelen(b->Sat, 1, Nav) == 0.0))
        {
            tracet(0, "CMR: internal error; satwavelen() failure.\n");
            return -1;
        }
...
static void SetStationCoordinates(raw_t *Raw, unsigned char *p)
{
    ...
    tracet(3,
           "CMR: Reference station coordinates received. X=%f, Y=%f, Z=%f, East offset=%f, North offset=%f, Up "
           "offset=0.0, Height=%f\n",
