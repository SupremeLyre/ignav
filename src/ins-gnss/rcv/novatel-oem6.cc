...
static int decode_bestpos(raw_t *raw)
{
    ...
    memcpy(sta_id, p, sizeof(unsigned char) * 4);
    p += 4;
...
static int decode_bestxyz(raw_t *raw)
{
    ...
    sol.sol_vel.v[0] = R8(p);
    p += 8;
    sol.sol_vel.v[1] = R8(p);
    p += 8;
    sol.sol_vel.v[2] = R8(p);
    p += 8;

    sol.sol_vel.qv[0] = R4(p);
    p += 4;
    sol.sol_vel.qv[1] = R4(p);
    p += 4;
    sol.sol_vel.qv[2] = R4(p);

    p += 4;
    p += 4; /* base station identification */
    v_latency = R4(p);
    p += 4;
...
static int decode_oem6_sol(raw_t *raw)
{
    ...
    if (raw->outtype)
    {
        sprintf(raw->msgtype, "OEM6 %4d (%4d): msg=%d %s", type, raw->len, msg, time_str(gpst2time(week, tow), 2));
    }
    if (msg != 0)
    ...
    {
    case ID_BESTPOS:
        return decode_bestpos(raw);
    case ID_BESTVEL:
        return decode_bestvel(raw);
    case ID_BESTXYZ:
        return decode_bestxyz(raw);
    }
    return 0;
}
...
static int decode_oem6_pose(raw_t *raw)
{
    ...
    case ID_HEAD:
        return decode_head(raw);
    case ID_HEAD2:
        return decode_head2(raw);
    case ID_HEADRATE:
        return decode_headrate(raw);
    }
    return 0;
}
    ...
    if (raw->nbyte == 0)
    {
        for (i = 0;; i++)
        {
            if ((data = fgetc(fp)) == EOF)
                return -2;
            if (sync_oem6(raw->buff, (unsigned char)data))
                break;
            if (i >= 4096)
                return 0;
        }
