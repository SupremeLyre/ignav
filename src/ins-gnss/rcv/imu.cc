...
extern void odores(double rese)
{
    res = rese;
}
/* set odometry wheel diameter------------------------------------------------*/
extern void odod(double de)
{
    d = de;
}
...
static void adjtime(raw_t *raw, const double sowi, double *sowo, double *timu, int *week, unsigned int *dcc)
{
...
static int decode_odo_data(raw_t *raw, double dt)
{
    ...
    dc = I2(raw->buff + 38) - raw->imu.odoc <= SHRT_MIN   ? I2(raw->buff + 38) - raw->imu.odoc + USHRT_MAX
         : I2(raw->buff + 38) - raw->imu.odoc >= SHRT_MAX ? I2(raw->buff + 38) - raw->imu.odoc - USHRT_MAX
...
static int decode_imu_data(raw_t *raw)
{
    ...
    if (dc * 1.0 / FREQOCXO > MAXDIFFTIME)
        return 0;
...
static int decode_imu_m39(raw_t *raw, unsigned char data)
{
    ...
    if (raw->nbyte < 2)
        return 0; /* synchronize frame */

    if (!chkhead(raw->buff, gi310_head))
    {
        raw->nbyte = 0;
        return 0;
    }
    if (raw->nbyte < NUMBYTES_GI310)
        return 0;

    if (chksum(raw->buff + 2, 40) == data)
    { /* checksum */
        raw->nbyte = 0;
...
static int nextimub(const imu_t *imu, imu_t *data, int *index)
{
    ...
    data->n = 0;
    for (i = 0; i < 100; i++)
        addimudata(data, &imu->data[(*index)--]);
    if (data->n)
        return 4;
    else
        return 0;
...
static int decode_imu_m39b(raw_t *raw, unsigned char data)
{
    if (raw->imub.n == 0)
    {
        ...
        readimub(str->path, &raw->imub, opt->insopt.imudecfmt, opt->insopt.imuformat, opt->insopt.imucoors,
                 opt->insopt.imuvalfmt);
...
extern int input_m39(raw_t *raw, unsigned char data)
{
    ...
    if (raw->dire)
        return decode_imu_m39b(raw, data);
    else
        return decode_imu_m39(raw, data);
}
...
extern int input_m39f(raw_t *raw, FILE *fp)
{
    ...
    for (i = 0; i < 4096; i++)
    {
        if ((data = fgetc(fp)) == EOF)
            return -2;
        if ((ret = input_m39(raw, (unsigned char)data)))
            return ret;
    }
 ...
 * --------------------------------------------------------------------------*/
extern int readimub(const char *file, imu_t *imu, int decfmt, int imufmt, int coor, int valfmt)
{
    FILE *fp;
    raw_t raw = {0};
    int data, siz;
