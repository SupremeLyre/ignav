...
static int inputimu(const imu_t *imu, imud_t *imudata, const prcopt_t *opt, imud_t *imuz, int ws)
{
    int i, k;
...
static int inputpos(const gsof_data_t *pos, gsof_t *gsofdata, gtime_t timu, const prcopt_t *opt)
{
    int flag = 0, i;
...
static void torts(ins_sol_t *sol, const insstate_t *ins, const insopt_t *opt, int type)
{
    if (sol->Pc == NULL || sol->Pp == NULL)
        return;
    if (type == 1)
    { /* updated/predicted ins state */
        matcpy(sol->pCbe, ins->Cbe, 3, 3);
        matcpy(sol->pre, ins->re, 3, 1);
        matcpy(sol->pve, ins->ve, 3, 1);
        matcpy(sol->pae, ins->ae, 3, 1);
        matcpy(sol->pba, ins->ba, 3, 1);
        matcpy(sol->pbg, ins->bg, 3, 1);
        matcpy(sol->pMa, ins->Ma, 3, 3);
        matcpy(sol->pMg, ins->Mg, 3, 3);
    ...
    { /* current ins state */
        ...
        matcpy(sol->cba, ins->ba, 3, 1);
        matcpy(sol->cbg, ins->bg, 3, 1);
        matcpy(sol->cMa, ins->Ma, 3, 3);
        matcpy(sol->cMg, ins->Mg, 3, 3);
 ...
 *---------------------------------------------------------------------------*/
static int fwdfilt(const imu_t *imu, const gsof_data_t *pos, const prcopt_t *popt, const solopt_t *solopt, rtk_t *rtk)
{
    ...
    while (inputimu(imu, &imus, popt, imuz, ws))
    {
        if (inputpos(pos, &poss, imus.time, popt))
        {
            ...
            {
                /* initialization */
                if (!(init = init_ins(&imus, &poss, iopt, ins)))
                    continue;
            }
            else
            {
...
static void err_corr(const ins_sol_t *pre, ins_sol_t *cur, const insopt_t *opt, double *x, double *P)
{
    double omg[3], dC[9];
...
static int upd_ins_rts(const ins_sol_t *pre, ins_sol_t *cur, insstate_t *ins, const insopt_t *opt)
{
    ...
    lcclp(xs, cur->cCbe, cur->cre, cur->cve, NULL, NULL, NULL, cur->sre, cur->sve, cur->sae, cur->sba, cur->sbg,
          cur->sMa, cur->sMg, cur->slever, cur->sCbe, NULL, NULL, opt);
...
static int bwdsmh(rtk_t *rtk, const prcopt_t *popt, const solopt_t *solopt)
{
    ...
    if (!(fp_fwd_sol = fopen(solfile, "rb")))
        return 0;
    ...
    for (i = insbuf.n - 2; i >= 0; i--)
    {
        ...
        pre = &fins[j % 2];
        j++;
        if (!get_ins_state(i + 1, bsize, &fins[j % 2], &pfins[j % 2]))
            continue;
        cur = &fins[j % 2];
#endif
 ...
 * return: number of epochs
 * --------------------------------------------------------------------------*/
extern int lcrts(const imu_t *imu, const gsof_data_t *pos, const prcopt_t *popt, const solopt_t *solopt, int port,
                 const char *file)
{
    rtk_t rtk = {0};
    int n;
    ...
    /* forward filter */
    if (fwdfilt(imu, pos, popt, solopt, &rtk))
        bwdsmh(&rtk, popt, solopt);
    if (!(n = insbuf.n))
    {
