...
static unsigned int outmoni = 0; /* output solution monitor */
...
/* ins states to covariance---------------------------------------------------*/
extern void insstatocov(const insopt_t *opt, const insstate_t *ins, double *Pa, double *Pv, double *Pp)
{
    int i, j;
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
        {
            if (Pa)
                Pa[i + j * 3] = opt->updint == UPDINT_IMU ? ins->P[(i + 0) + (j + 0) * ins->nx]
                                                          : ins->Pa[(i + 0) + (j + 0) * ins->nx];
            if (Pv)
                Pv[i + j * 3] = opt->updint == UPDINT_IMU ? ins->P[(i + 3) + (j + 3) * ins->nx]
                                                          : ins->Pa[(i + 3) + (j + 3) * ins->nx];
            if (Pp)
                Pp[i + j * 3] = opt->updint == UPDINT_IMU ? ins->P[(i + 6) + (j + 6) * ins->nx]
                                                          : ins->Pa[(i + 6) + (j + 6) * ins->nx];
        }
    if (ins->gstat == SOLQ_FIX && ((prcopt_t *)opt->gopt)->mode == PMODE_INS_TGNSS)
    {
        for (i = 0; i < 3; i++)
            ...
            {
                if (Pa)
                    Pa[i + j * 3] = ins->Pb[(i + 0) + (j + 0) * ins->nb];
                if (Pv)
                    Pv[i + j * 3] = ins->Pb[(i + 3) + (j + 3) * ins->nb];
                if (Pp)
                    Pp[i + j * 3] = ins->Pb[(i + 6) + (j + 6) * ins->nb];
            }
    }
}
...
static int test_nmea(const char *buff)
{
    if (strlen(buff) < 6 || buff[0] != '$')
        return 0;
    return !strncmp(buff + 1, "GP", 2) || !strncmp(buff + 1, "GA", 2) || !strncmp(buff + 1, "GL", 2) ||
           !strncmp(buff + 1, "GN", 2) || !strncmp(buff + 1, "GB", 2) || !strncmp(buff + 1, "BD", 2) ||
           !strncmp(buff + 1, "QZ", 2);
}
...
/* decode solution position mode----------------------------------------------*/
static int decode_solposmode(char *buff)
{
    trace(4, "decode_solposmode:\n");

    if (strstr(buff, GNSPOSSTR))
        return 1;
    if (strstr(buff, INSPOSSTR))
        return 2;
    if (strstr(buff, VOSPOSSTR))
        return 3;
    if (strstr(buff, GTHPOSSTR))
        return 4;
    return 0;
}
/* decode x/y/z-ecef ---------------------------------------------------------*/
static int decode_solxyz(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    if (i < n)
        sol->stat = (unsigned char)val[i++];
    if (i < n)
        sol->ista = (unsigned char)val[i++];
    if (i < n)
        sol->ns = (unsigned char)val[i++];
    if (i + 3 <= n)
    {
        P[0] = val[i] * val[i];
        i++; /* sdx */
...
/* decode vo solution---------------------------------------------------------*/
static int decode_solvo(char *buff, const solopt_t *opt, sol_t *sol)
{
    const char *sep = opt2sep(opt);
    double val[MAXFIELD];
    int i = 0, j, n;

    trace(4, "decode_solvo:\n");
...
/* decode ins solution--------------------------------------------------------*/
static int decode_solins(char *buff, const solopt_t *opt, sol_t *sol)
{
    double val[MAXFIELD], P[9] = {0}, Q[9], pos[3], vel[3];
    int i = 0, j, n;
    const char *sep = opt2sep(opt);

    trace(4, "decode_solins: buff=%s\n", buff);

    if ((n = tonum(buff, sep, val)) < 3)
        return 0;

    if (opt->ins_posf == SOLF_XYZ)
    {
        for (j = 0; j < 3; j++)
            sol->rr[j] = val[i++]; /* xyz */
    }
    else if (opt->ins_posf == SOLF_LLH)
    {
        if (!opt->degf)
        {
            if (n < 3)
    ...
    /* velocity and its variance/covariance */
    if (opt->ins_posf == SOLF_XYZ)
    {
        if (i + 3 <= n)
        {
    ...
    if (i + 3 <= n && opt->outatt)
        for (j = 0; j < 3; j++)
            sol->att[j] = val[i++]; /* attitude */
    if (i + 3 <= n && opt->outatt)
        for (j = 0; j < 3; j++)
            sol->qa[j] = (float)val[i++]; /* attitude variance */
    if (i + 3 <= n && opt->outvb)
        for (j = 0; j < 3; j++)
            sol->vb[j] = val[i++]; /* vehicle velocity */
    if (i + 3 <= n && opt->outacc)
        for (j = 0; j < 3; j++)
            sol->ab[j] = val[i++]; /* vehicle acceleration */
    if (i + 3 <= n && opt->outba)
        for (j = 0; j < 3; j++)
            sol->ba[j] = val[i++]; /* acceleration bias */
    if (i + 3 <= n && opt->outbg)
        for (j = 0; j < 3; j++)
            sol->bg[j] = val[i++]; /* gyro bias */
    if (i + 3 <= n && opt->odo)
        for (j = 0; j < 3; j++)
            sol->vr[j] = val[i++]; /* odometry velocity in rear frame */
    if (i + 1 <= n && opt->odo)
        for (j = 0; j < 1; j++)
            sol->os = val[i++]; /* odometry scale factor */
    if (i + 4 <= n && opt->outclk)
        for (j = 0; j < 4; j++)
            sol->dtr[j] = val[i++]; /* receiver clock bias to time systems (s) */
    if (i + 3 <= n && opt->dopp)
        for (j = 0; j < 3; j++)
            sol->dv[j] = val[i++]; /* doppler velocity expressed in ecef */
    if (i + 1 <= n && opt->dopp)
        for (j = 0; j < 1; j++)
            sol->dtrr = val[i++]; /* receiver clock drift (m/s) about GPST */
    if (i + 6 <= n && opt->outimuraw)
    {
        for (j = 0; j < 3; j++)
            sol->imu.gyro[j] = val[i++]; /* imu gyro measurement data */
        for (j = 0; j < 3; j++)
            sol->imu.accl[j] = val[i++]; /* imu accl measurement data */
    }
    for (i = 0; i < 3; i++)
        sol->qa[i] = SQR(sol->qa[i]);
    return 1;
}
/* decode ground truth solution-----------------------------------------------*/
static int decode_groundtruth(char *buff, const solopt_t *opt, sol_t *sol)
{
    sol_t sol0 = {{0}};
    char *p = buff;
    const char *sep = opt2sep(opt);
    ...
    trace(4, "decode_groundtruth: buff=%s\n", buff);
...
static int decode_solpos(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    *sol = sol0;
    ...
    case SOLF_INS:
        return decode_solins(p, opt, sol);
    case SOLF_VO:
        return decode_solvo(p, opt, sol);
    }
    return 0;
}
 ...
 *-----------------------------------------------------------------------------*/
extern int inputsol(unsigned char data, gtime_t ts, gtime_t te, double tint, int qflag, const solopt_t *opt,
                    solbuf_t *solbuf)
{
    sol_t sol = {{0}};
    int stat;
 ...
 *          solbuf_t *solbuf IO solution buffer
 * return : status (1:solution received,0:no solution,-1:disconnect received)
 * note : this function only use in decode solution from monitor stream
 *-----------------------------------------------------------------------------*/
extern int inputsolx(unsigned char data, gtime_t ts, gtime_t te, double tint, int qflag, solbuf_t *solbuf)
{
    sol_t sol = {{0}};
    const solopt_t *solopt = &solopt_default;
    int stat;

    trace(4, "inputsolx: data=0x%02x\n", data);
    ...
    /* decode position mode */
    stat = decode_solposmode((char *)solbuf->buff);
    switch (stat)
    {
    case 0:
    ...
    case 1:
        ...
        break;
    case 2:
        solopt = &solopt_ins_default;
        break;
    case 3:
        solopt = &solopt_vo_default;
        break;
    case 4:
        solopt = &solopt_default;
        goto ground_truth;
    ...
    /* decode solution */
    sol.time = solbuf->time;
    if ((stat = decode_sol((char *)solbuf->buff, solopt, &sol, solbuf->rb)) > 0)
    {
        if (stat)
            solbuf->time = sol.time; /* update current time */
        if (stat != 1)
...
ground_truth:
    /* decode ground truth solution */
    if (decode_groundtruth((char *)solbuf->buff, solopt, &sol))
    {
        return addsol(solbuf, &sol);
    }
    else
...
extern int readsolx(char *files[], int nfile, gtime_t ts, gtime_t te, double tint, int qflag, const solopt_t *opt,
                    solbuf_t *solbuf)
{
    FILE *fp;
    int i;

    trace(3, "readsoltx: nfile=%d\n", nfile);
    ...
    for (i = 0; i < nfile; i++)
    {
        if (!(fp = fopen(files[i], "rb")))
        {
            trace(2, "readsoltx: file open error %s\n", files[i]);
            continue;
        }
        /* read solution options in header */
        rewind(fp);
        ...
        if (!readsoldata(fp, ts, te, tint, qflag, opt, solbuf))
        {
            trace(2, "readsoltx: no solution in %s\n", files[i]);
        }
        fclose(fp);
    }
    return sort_solbuf(solbuf);
...
/* free pose measurement buffer-----------------------------------------------*/
extern void freeposebuf(posebuf_t *posebuf)
{
    trace(3, "freeposebuf: n=%d\n", posebuf->n);

    if (posebuf->data)
        free(posebuf->data);
    posebuf->n = posebuf->nmax = posebuf->start = posebuf->end = 0;
    posebuf->data = NULL;
}
/* initial pose measurement buffer--------------------------------------------
 * initialize pose measurement buffer
 * args   : posebuf_t *posebuf I  pose measurement buffer
 *          int    cyclic      I  pose measurement buffer type (0:linear,1:cyclic)
 *          int    nmax        I  initial number of pose measurement
 * return : status (1:ok,0:error)
 *-----------------------------------------------------------------------------*/
extern void initposebuf(posebuf_t *posebuf, int cyclic, int nmax)
{
#if 0
    gtime_t time0={0};
#endif
    trace(3, "initposebuf: cyclic=%d nmax=%d\n", cyclic, nmax);

    posebuf->n = posebuf->nmax = posebuf->start = posebuf->end = 0;
    posebuf->cyclic = cyclic;
#if 0
    solbuf->time=time0;
#endif
    posebuf->data = NULL;
    if (cyclic)
    {
        if (nmax <= 2)
            nmax = 2;
        if (!(posebuf->data = (pose_meas_t *)malloc(sizeof(pose_meas_t) * nmax)))
        {
            trace(1, "initposebuf: memory allocation error\n");
            return;
        }
        posebuf->nmax = nmax;
    }
}
/* add pose data to pose measurement buffer ----------------------------------
 * add pose data to pose measurement buffer
 * args   : solbuf_t *solbuf IO solution buffer
 *          sol_t  *sol      I  solution data
 * return : status (1:ok,0:error)
 *-----------------------------------------------------------------------------*/
extern int addpose(posebuf_t *posebuf, const pose_meas_t *pose)
{
    pose_meas_t *solbuf_data;
    ...
    { /* ring buffer */
        if (posebuf->nmax <= 1)
            return 0;
        posebuf->data[posebuf->end] = *pose;
        if (++posebuf->end >= posebuf->nmax)
            posebuf->end = 0;
        if (posebuf->start == posebuf->end)
        {
            if (++posebuf->start >= posebuf->nmax)
                posebuf->start = 0;
        }
        else
            posebuf->n++;
    ...
    if (posebuf->n >= posebuf->nmax)
    {
        posebuf->nmax = posebuf->nmax == 0 ? 8192 : posebuf->nmax * 2;
        if (!(solbuf_data = (pose_meas_t *)realloc(posebuf->data, sizeof(pose_meas_t) * posebuf->nmax)))
        {
            trace(1, "addpose: memory allocation error\n");
            free(posebuf->data);
            posebuf->data = NULL;
            posebuf->n = posebuf->nmax = 0;
            return 0;
        }
        posebuf->data = solbuf_data;
    }
    posebuf->data[posebuf->n++] = *pose;
    return 1;
}
...
static int decode_solstat(char *buff, solstat_t *stat)
{
    ...
    n = sscanf(buff, "$SAT%d%lf%s%d%lf%lf%lf%lf%d%d%d%d%d%d%d%d", &week, &tow, id, &frq, &az, &el, &resp, &resc, &vsat,
...
/* output vo solution---------------------------------------------------------*/
static int outvo(unsigned char *buff, const sol_t *sol, const char *s, const solopt_t *opt)
{
    const char *sep = opt2sep(opt);
    char *p = (char *)buff;
    ...
    p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f", s, sep, sol->rr[0], sep, sol->rr[1], sep,
                 sol->rr[2]); /* output position */
    p +=
        sprintf(p, "%s%10.5f%s%10.5f%s%10.5f", sep, sol->rr[3], sep, sol->rr[4], sep, sol->rr[5]); /* output velocity */
    p += sprintf(p, "%s%10.5f%s%10.5f%s%10.5f", sep, sol->att[0] * R2D, sep, sol->att[1] * R2D, sep,
                 sol->att[2] * R2D); /* output attitude */

    if (outmoni)
        p += sprintf(p, " %s", VOSPOSSTR);
    p += sprintf(p, "\n");
    return p - (char *)buff;
}
/* output ins solution -------------------------------------------------------*/
static int outins(unsigned char *buff, const sol_t *sol, const char *s, const solopt_t *opt)
{
    const char *sep = opt2sep(opt);
    char *p = (char *)buff;
    ...
    if (opt->ins_posf == SOLF_LLH)
    {
        ...
        if (opt->degf)
        {
            ...
            p += sprintf(p, "%s%s%4.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f", s, sep, dms1[0], sep, dms1[1], sep,
                         dms1[2], sep, dms2[0], sep, dms2[1], sep, dms2[2]);
        }
        else
        {
        ...
        if (opt->outvel)
        { /* output velocity */
            ecef2enu(pos, sol->rr + 3, venu);
            p += sprintf(p, "%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f", sep, venu[1],
                         sep, venu[0], sep, venu[2], sep, SQRT(Qv[4]), sep, SQRT(Qv[0]), sep, SQRT(Qv[8]), sep,
                         sqvar(Qv[1]), sep, sqvar(Qv[2]), sep, sqvar(Qv[5]));
        }
    }
    else if (opt->ins_posf == SOLF_XYZ)
    {
        ...
        if (opt->outvel)
        { /* output velocity */
            p += sprintf(p, "%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f", sep, sol->rr[3],
                         sep, sol->rr[4], sep, sol->rr[5], sep, SQRT(Pv[0]), sep, SQRT(Pv[4]), sep, SQRT(Pv[8]), sep,
                         sqvar(Pv[1]), sep, sqvar(Pv[2]), sep, sqvar(Pv[5]));
        }
    }
    if (opt->outatt)
    { /* output attitude */
    ...
    if (opt->outvb)
    {
        p += sprintf(p, "%s%10.4lf%s%10.4lf%s%10.4lf", sep, sol->vb[0], sep, sol->vb[1], sep, sol->vb[2]);
    }
    if (opt->outacc)
    {
        p += sprintf(p, "%s%10.4lf%s%10.4lf%s%10.4lf", sep, sol->ab[0], sep, sol->ab[1], sep, sol->ab[2]);
    }
    if (opt->outba)
    {
        p += sprintf(p, "%s%10.6lf%s%10.6lf%s%10.6lf", sep, sol->ba[0], sep, sol->ba[1], sep, sol->ba[2]);
    }
    if (opt->outbg)
    {
        p += sprintf(p, "%s%10.6lf%s%10.6lf%s%10.6lf", sep, sol->bg[0], sep, sol->bg[1], sep, sol->bg[2]);
    }
    if (opt->odo)
    {
        p += sprintf(p, "%s%10.6lf%s%10.6lf%s%10.6lf%s%10.6lf", sep, sol->vr[0], sep, sol->vr[1], sep, sol->vr[2], sep,
                     sol->os);
    }
    if (opt->outclk)
    {
        p += sprintf(p, "%s%15.6lf%s%15.6lf%s%15.6lf%s%15.6lf", sep, sol->dtr[0] * 1E3, sep, sol->dtr[1] * 1E3, sep,
                     sol->dtr[2] * 1E3, sep, sol->dtr[3] * 1E3);
    }
    if (opt->dopp)
    {
        p += sprintf(p, "%s%10.6lf%s%10.6lf%s%10.6lf%s%10.6lf", sep, sol->dv[0], sep, sol->dv[1], sep, sol->dv[2], sep,
                     sol->dtrr);
    }
    if (opt->outimuraw)
    {
        p += sprintf(p, "%s%12.6lf%s%12.6lf%s%12.6lf%s%12.6lf%s%12.6lf%s%12.6lf", sep, sol->imu.gyro[0] * R2D, sep,
                     sol->imu.gyro[1] * R2D, sep, sol->imu.gyro[2] * R2D, sep, sol->imu.accl[0], sep, sol->imu.accl[1],
                     sep, sol->imu.accl[2]);
    }
    if (outmoni)
        p += sprintf(p, " %s", INSPOSSTR);
    p += sprintf(p, "\n");
    return p - (char *)buff;
}
...
static int outecef(unsigned char *buff, const char *s, const sol_t *sol, const solopt_t *opt)
{
    ...
    { /* output velocity */
        p += sprintf(p, "%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f", sep, sol->rr[3],
                     sep, sol->rr[4], sep, sol->rr[5], sep, SQRT(sol->qv[0]), sep, SQRT(sol->qv[1]), sep,
                     SQRT(sol->qv[2]), sep, sqvar(sol->qv[3]), sep, sqvar(sol->qv[4]), sep, sqvar(sol->qv[5]));
    }
    if (outmoni)
        p += sprintf(p, " %s", GNSPOSSTR);
    p += sprintf(p, "\n");
    return p - (char *)buff;
}
...
static int outpos(unsigned char *buff, const char *s, const sol_t *sol, const solopt_t *opt)
{
    ...
    { /* output velocity */
        ...
        covenu(pos, P, Q);
        p += sprintf(p, "%s%10.5f%s%10.5f%s%10.5f%s%9.5f%s%8.5f%s%8.5f%s%8.5f%s%8.5f%s%8.5f", sep, vel[1], sep, vel[0],
                     sep, vel[2], sep, SQRT(Q[4]), sep, SQRT(Q[0]), sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[2]),
                     sep, sqvar(Q[5]));
    }
    if (outmoni)
    {
...
static int outenu(unsigned char *buff, const char *s, const sol_t *sol, const double *rb, const solopt_t *opt)
{
    ...
    p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n", s,
                 sep, enu[0], sep, enu[1], sep, enu[2], sep, sol->stat, sep, sol->ns, sep, SQRT(Q[0]), sep, SQRT(Q[4]),
                 sep, SQRT(Q[8]), sep, sqvar(Q[1]), sep, sqvar(Q[5]), sep, sqvar(Q[2]), sep, sol->age, sep, sol->ratio);
    if (opt->wlratio)
    {
        p += sprintf(p, "%s%6.1f", sep, sol->wlratio);
    }
...
extern int outnmea_rmc(unsigned char *buff, const sol_t *sol)
{
    ...
    p += sprintf(
        p, "$GPRMC,%02.0f%02.0f%05.2f,A,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%4.2f,%4.2f,%02.0f%02.0f%02d,%.1f,%s,%s",
        ep[3], ep[4], ep[5], dms1[0], dms1[1] + dms1[2] / 60.0, pos[0] >= 0 ? "N" : "S", dms2[0],
        dms2[1] + dms2[2] / 60.0, pos[1] >= 0 ? "E" : "W", vel / KNOT2M, dir, ep[2], ep[1], (int)ep[0] % 100, amag,
        emag, sol->stat == SOLQ_DGPS || sol->stat == SOLQ_FLOAT || sol->stat == SOLQ_FIX ? "D" : "A");
    for (q = (char *)buff + 1, sum = 0; *q; q++)
        sum ^= *q; /* check-sum */
    p += sprintf(p, "*%02X%c%c", sum, 0x0D, 0x0A);
    return p - (char *)buff;
...
extern int outnmea_gga(unsigned char *buff, const sol_t *sol)
{
    ...
    p += sprintf(p, "$GPGGA,%02.0f%02.0f%05.2f,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%d,%02d,%.1f,%.3f,M,%.3f,M,%.1f,",
                 ep[3], ep[4], ep[5], dms1[0], dms1[1] + dms1[2] / 60.0, pos[0] >= 0 ? "N" : "S", dms2[0],
                 dms2[1] + dms2[2] / 60.0, pos[1] >= 0 ? "E" : "W", solq, sol->ns, dop, pos[2] - h, h, sol->age);
    for (q = (char *)buff + 1, sum = 0; *q; q++)
        sum ^= *q; /* check-sum */
    p += sprintf(p, "*%02X%c%c", sum, 0x0D, 0x0A);
    return p - (char *)buff;
...
extern int outprcopts(unsigned char *buff, const prcopt_t *opt)
{
    ...
    const char *s2[] = {"L1", "L1+L2", "L1+L2+L5", "L1+L2+L5+L6", "L1+L2+L5+L6+L7", "L1+L2+L5+L6+L7+L8", ""};
...
/* output vo solution header---------------------------------------------------*/
static int outvosolhead(unsigned char *buff, const solopt_t *opt)
{
    trace(3, "outvosolhead:\n");
    ...
    p += sprintf(p, "%14s%s%14s%s%14s", "x-ecef(m)", sep, "y-ecef(m)", sep, "z-ecef(m)");
    p += sprintf(p, "%s%10s%s%10s%s%10s", sep, "vx(m/s)", sep, "vy(m/s)", sep, "vz(m/s)");
    p += sprintf(p, "%s%10s%s%10s%s%10s", sep, "roll(deg)", sep, "pitch(deg)", sep, "yaw(deg)");
    p += sprintf(p, "\n");
    return p - (char *)buff;
}
 ...
 *          insopt_t *insopt I   ins states options
 * return : number of output bytes
 *-----------------------------------------------------------------------------*/
extern int outsolheads(unsigned char *buff, const solopt_t *opt)
{
    ...
    if (opt->outhead)
    {
        ...
        p += sprintf(p, ",Q=1:fix,2:float,3:sbas,4:dgps,5:single,6:ppp,ns=# of satellites),"
                        "Qins=1:ins mechanization,2:ins mechanization and propagate states and covariance,3:ins-gnss "
                        "loosely-coupled updates\n");
    }
    p += sprintf(p, "%s  %-*s%s", COMMENTH, (opt->timef ? 16 : 8) + timeu + 1, s3[opt->times], sep);

    if (opt->posf == SOLF_LLH || (opt->posf == SOLF_INS && opt->ins_posf == SOLF_LLH))
    { /* lat/lon/hgt */
        if (opt->degf)
        {
            p +=
        ...
        {
            p += sprintf(p, "%s%10s%s%10s%s%10s%s%10s%s%10s%s%10s", sep, "roll(deg)", sep, "pitch(deg)", sep,
                         "yaw(deg)", sep, "sdroll", sep, "sdpitch", sep, "sdyaw");
        }
        if (opt->outvb && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%10s%s%10s%s%10s", sep, "vbx(m/s)", sep, "vby(m/s)", sep, "vbz(m/s)");
        }
        if (opt->outacc && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%10s%s%10s%s%10s", sep, "abx(m/s^2)", sep, "aby(m/s^2)", sep, "abz(m/s^2)");
        }
        if (opt->outba && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%10s%s%10s%s%10s", sep, "bax(m/s^2)", sep, "bay(m/s^2)", sep, "baz(m/s^2)");
        }
        if (opt->outbg && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%10s%s%10s%s%10s", sep, "bgx(rad/s)", sep, "bgy(rad/s)", sep, "bgz(rad/s)");
        }
        if (opt->odo && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%10s%s%10s%s%10s", sep, "vox(m/s)", sep, "voy(m/s)", sep, "voz(m/s)");
            p += sprintf(p, "%s%10s", sep, "odo-s");
        }
        if (opt->outclk && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%15s%s%15s%s%15s%s%15s", sep, "clk-gps(ms)", sep, "clk-glo-gps(ms)", sep,
                         "clk-gal-gps(ms)", sep, "clk-cmp-gps(ms)");
        }
        if (opt->dopp && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%10s%s%10s%s%10s%s%10s", sep, "dopp-vx(m/s)", sep, "dopp-vy(m/s)", sep, "dopp-vz(m/s)",
                         sep, "clk-dri(m/s)");
        }
        if (opt->outimuraw && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%12s%s%12s%s%12s%s%12s%s%12s%s%12s", sep, "gyrox(deg)", sep, "gyroy(deg)", sep,
                         "gyroz(deg)", sep, "acclx(m/s^2)", sep, "accly(m/s^2)", sep, "acclz(m/s^2)");
        }
    }
    ...
    { /* x/y/z-ecef */
        ...
        {
            p +=
                sprintf(p, "%s%10s%s%10s%s%10s%s%10s", sep, "vox(m/s)", sep, "voy(m/s)", sep, "voz(m/s)", sep, "odo-s");
        }
        if (opt->outclk && opt->posf == SOLF_INS)
        {
            p += sprintf(p, "%s%15s%s%15s%s%15s%s%15s", sep, "clk-gps(ms)", sep, "clk-glo-gps(ms)", sep,
 ...
 *          insopt_t *insopt I   ins options
 *          int type         I   output monitor/no output monitor
 * return : number of output bytes
 *-----------------------------------------------------------------------------*/
extern int outsols(unsigned char *buff, const sol_t *sol, const double *rb, const solopt_t *opt, const insstate_t *ins,
                   const insopt_t *insopt, int type)
{
    gtime_t time, ts = {0};
    double gpst;
    int week, timeu;
    ...
    outmoni = (unsigned int)type; /* set flag of output monitor */

    /* output visual odometry solution */
    if (opt->posf == SOLF_VO)
        goto vosols;
    ...
    if (opt->posf == SOLF_INS && ins == NULL)
    {
        trace(2, "output ins states have some unknown problem\n");
        return 0;
    }
vosols:
...
/* output ground truth position solution for stream monitor--------------------*/
extern int outgroundtruth(unsigned char *buff, const sol_t *sol, const solopt_t *opt)
{
 ...
 *-----------------------------------------------------------------------------*/
extern int outsolexs(unsigned char *buff, const sol_t *sol, const ssat_t *ssat, const solopt_t *opt, int type)
{
    gtime_t ts = {0};
    unsigned char *p = buff;
    ...
    /* set flag of output monitor */
    outmoni = (unsigned int)type;
 ...
 * return : none
 *-----------------------------------------------------------------------------*/
extern void outsolhead(FILE *fp, const solopt_t *opt, const insopt_t *insopt)
{
    unsigned char buff[MAXSOLMSG + 1];
    int n;
 ...
 *          double *rb       I   base station position {x,y,z} (ecef) (m)
 *          solopt_t *opt    I   solution options
 *          insopt_t *insopt I   ins solution options
 * return : none
 *-----------------------------------------------------------------------------*/
extern void outsol(FILE *fp, const sol_t *sol, const double *rb, const solopt_t *opt, const insstate_t *ins,
                   const insopt_t *insopt)
{
    ...
    if ((n = outsols(buff, sol, rb, opt, ins, insopt, 0)) > 0)
    {
        fwrite(buff, n, 1, fp);
    }
}
...
extern void outsolex(FILE *fp, const sol_t *sol, const ssat_t *ssat, const solopt_t *opt)
{
    ...
    if ((n = outsolexs(buff, sol, ssat, opt, 0)) > 0)
    {
        fwrite(buff, n, 1, fp);
    }
}
