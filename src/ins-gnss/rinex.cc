...
/* read rinex header ---------------------------------------------------------*/
static int readrnxh(FILE *fp, double *ver, char *type, int *sys, int *tsys, char tobs[][MAXOBSTYPE][4], nav_t *nav,
                    sta_t *sta)
{
    double bias;
    ...
    *sys = SYS_GPS;
    ...
    while (fgets(buff, MAXRNXLEN, fp))
    {
        ...
        { /* opt */
            ...
            {
                /* cnes/cls grg clock */
                if (!strncmp(buff, "WL", 2) && (sat = satid2no(buff + 3)) && sscanf(buff + 40, "%lf", &bias) == 1)
                {
...
static int readrnxobsb(FILE *fp, const char *opt, double ver, int *tsys, char tobs[][MAXOBSTYPE][4], int *flag,
                       obsd_t *data, sta_t *sta, sigind_t *sigind)
{
    gtime_t time = {0};
    sigind_t index[7] = {{0}};
    char buff[MAXRNXLEN];
    int i = 0, n = 0, nsat = 0, sats[MAXOBS] = {0}, mask, j;
...
/* read rinex obs ------------------------------------------------------------*/
static int readrnxobs(FILE *fp, gtime_t ts, gtime_t te, double tint, const char *opt, int rcv, double ver, int *tsys,
                      char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta)
{
    ...
    /* read rinex obs data body */
    while ((n = readrnxobsb(fp, opt, ver, tsys, tobs, &flag, data, sta, obs->sind[rcv <= 2 ? rcv - 1 : 0])) >= 0 &&
           stat >= 0)
    {
 ...
 *
 *-----------------------------------------------------------------------------*/
extern int readrnxt(const char *file, int rcv, gtime_t ts, gtime_t te, double tint, const char *opt, obs_t *obs,
                    nav_t *nav, sta_t *sta)
{
    int i, n, stat = 0;
    const char *p;
...
extern int input_rnxctr(rnxctr_t *rnx, FILE *fp)
{
    ...
    if (rnx->type == 'O')
    {
        if ((n = readrnxobsb(fp, rnx->opt, rnx->ver, &rnx->tsys, rnx->tobs, &flag, rnx->obs.data, &rnx->sta,
                             rnx->obs.sind[0])) <= 0)
        {
            rnx->obs.n = 0;
            return n < 0 ? -2 : 0;
...
extern int outrnxobsh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    if (opt->rnxver <= 2.99)
    { /* ver.2 */
        strcpy(sys, opt->navsys == SYS_GPS ? "G (GPS)" : "M (MIXED)");
    }
    else
    { /* ver.3 */
        if (opt->navsys == SYS_GPS)
            ...
            strcpy(sys, "R: GLONASS");
        else if (opt->navsys == SYS_GAL)
            strcpy(sys, "E: Galielo");
        else if (opt->navsys == SYS_QZS)
            strcpy(sys, "J: QZSS"); /* ver.3.02 */
        else if (opt->navsys == SYS_CMP)
            strcpy(sys, "C: BeiDou"); /* ver.3.02 */
        else if (opt->navsys == SYS_IRN)
            strcpy(sys, "I: IRNSS"); /* ver.3.03 */
        else if (opt->navsys == SYS_SBS)
            strcpy(sys, "S: SBAS Payload");
        else
            strcpy(sys, "M: Mixed");
    }
...
static int obsindex(double ver, int sys, const unsigned char *code, const char *tobs, const char *mask)
{
    char *id;
    int i;
    ...
    for (i = 0; i < NFREQ + NEXOBS; i++)
    {
        ...
        { /* ver.2 */
            ...
            else if (ver >= 2.12 && tobs[1] == 'A')
            { /* L1C/A */
                if (code[i] == CODE_L1C)
                    return i;
            }
            else if (ver >= 2.12 && tobs[1] == 'B')
            { /* L1C */
                if (code[i] == CODE_L1S || code[i] == CODE_L1L || code[i] == CODE_L1X)
                    return i;
            }
            else if (ver >= 2.12 && tobs[1] == 'C')
            { /* L2C */
                if (code[i] == CODE_L2S || code[i] == CODE_L2L || code[i] == CODE_L2X)
                    return i;
            }
            else if (ver >= 2.12 && tobs[1] == 'D' && sys == SYS_GLO)
            { /* GLO L2C/A */
                if (code[i] == CODE_L2C)
                    return i;
            }
...
extern int outrnxobsb(FILE *fp, const rnxopt_t *opt, const obsd_t *obs, int n, int flag)
{
    ...
    for (i = ns = 0; i < n && ns < MAXOBS; i++)
    {
        ...
        if (!opt->nobs[opt->rnxver <= 2.99 ? 0 : s[ns]])
            continue;
        ind[ns++] = i;
    }
    if (opt->rnxver <= 2.99)
    { /* ver.2 */
        fprintf(fp, " %02d %2.0f %2.0f %2.0f %2.0f%11.7f  %d%3d", (int)ep[0] % 100, ep[1], ep[2], ep[3], ep[4], ep[5],
                flag, ns);
        for (i = 0; i < ns; i++)
        {
            if (i > 0 && i % 12 == 0)
...
extern int outrnxnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    { /* ver.2 */
        if (opt->outiono)
        {
            fprintf(fp, "  %12.4E%12.4E%12.4E%12.4E%10s%-20s\n", nav->ion_gps[0], nav->ion_gps[1], nav->ion_gps[2],
                    nav->ion_gps[3], "", "ION ALPHA");
            fprintf(fp, "  %12.4E%12.4E%12.4E%12.4E%10s%-20s\n", nav->ion_gps[4], nav->ion_gps[5], nav->ion_gps[6],
                    nav->ion_gps[7], "", "ION BETA");
        }
        if (opt->outtime)
        {
            fprintf(fp, "   ");
    ...
    { /* ver.3 */
        if (opt->navsys & SYS_GPS)
        {
            if (opt->outiono)
            {
                fprintf(fp, "GPSA %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_gps[0], nav->ion_gps[1],
                        nav->ion_gps[2], nav->ion_gps[3], "", "IONOSPHERIC CORR");
                fprintf(fp, "GPSB %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_gps[4], nav->ion_gps[5],
                        nav->ion_gps[6], nav->ion_gps[7], "", "IONOSPHERIC CORR");
            }
        }
        if (opt->navsys & SYS_GAL)
        {
            if (opt->outiono)
            {
                fprintf(fp, "GAL  %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_gal[0], nav->ion_gal[1],
                        nav->ion_gal[2], 0.0, "", "IONOSPHERIC CORR");
            }
        }
        if (opt->navsys & SYS_QZS)
        {
            if (opt->outiono)
            {
                fprintf(fp, "QZSA %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_qzs[0], nav->ion_qzs[1],
                        nav->ion_qzs[2], nav->ion_qzs[3], "", "IONOSPHERIC CORR");
                fprintf(fp, "QZSB %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_qzs[4], nav->ion_qzs[5],
                        nav->ion_qzs[6], nav->ion_qzs[7], "", "IONOSPHERIC CORR");
            }
        }
        if ((opt->navsys & SYS_IRN) && opt->rnxver >= 3.03)
        { /* ver.3.03 */
            if (opt->outiono)
            {
                fprintf(fp, "IRNA %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_irn[0], nav->ion_irn[1],
                        nav->ion_irn[2], nav->ion_irn[3], "", "IONOSPHERIC CORR");
                fprintf(fp, "IRNB %12.4E%12.4E%12.4E%12.4E%7s%-20s\n", nav->ion_irn[4], nav->ion_irn[5],
                        nav->ion_irn[6], nav->ion_irn[7], "", "IONOSPHERIC CORR");
            }
        }
        if (opt->navsys & SYS_GPS)
        {
            if (opt->outtime)
            {
                fprintf(fp, "GPUT %17.10E%16.9E%7.0f%5.0f %-5s %-2s %-20s\n", nav->utc_gps[0], nav->utc_gps[1],
                        nav->utc_gps[2], nav->utc_gps[3], "", "", "TIME SYSTEM CORR");
            }
        }
        if (opt->navsys & SYS_GAL)
        {
            if (opt->outtime)
            {
                fprintf(fp, "GAUT %17.10E%16.9E%7.0f%5.0f %-5s %-2s %-20s\n", nav->utc_gal[0], nav->utc_gal[1],
                        nav->utc_gal[2], nav->utc_gal[3], "", "", "TIME SYSTEM CORR");
            }
        }
        if ((opt->navsys & SYS_QZS) && opt->rnxver >= 3.02)
        { /* ver.3.02 */
            if (opt->outtime)
            {
                fprintf(fp, "QZUT %17.10E%16.9E%7.0f%5.0f %-5s %-2s %-20s\n", nav->utc_qzs[0], nav->utc_qzs[1],
                        nav->utc_qzs[2], nav->utc_qzs[3], "", "", "TIME SYSTEM CORR");
            }
        }
        if ((opt->navsys & SYS_CMP) && opt->rnxver >= 3.02)
        { /* ver.3.02 */
            if (opt->outtime)
            {
                fprintf(fp, "BDUT %17.10E%16.9E%7.0f%5.0f %-5s %-2s %-20s\n", nav->utc_cmp[0], nav->utc_cmp[1],
                        nav->utc_cmp[2], nav->utc_cmp[3], "", "", "TIME SYSTEM CORR");
            }
        }
        if ((opt->navsys & SYS_IRN) && opt->rnxver >= 3.03)
        { /* ver.3.03 */
            if (opt->outtime)
            {
                fprintf(fp, "IRUT %17.10E%16.9E%7.0f%5.0f %-5s %-2s %-20s\n", nav->utc_irn[0], nav->utc_irn[1],
                        nav->utc_irn[2], nav->utc_irn[3], "", "", "TIME SYSTEM CORR");
            }
        }
    }
    if (opt->outleaps)
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
    ...
    if (opt->rnxver > 2.99 || sys == SYS_GAL || sys == SYS_CMP)
    { /* ver.3 or ver.2 GAL */
    ...
    { /* ver.2 or ver.3.02 QZS */
        if (!sat2code(eph->sat, code))
            return 0;
        fprintf(fp, "%-3s %02d %2.0f %2.0f %2.0f %2.0f %4.1f", code, (int)ep[0] % 100, ep[1], ep[2], ep[3], ep[4],
                ep[5]);
        sep = "    ";
    }
    else
...
extern int outrnxgnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    { /* ver.3 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver, "N: GNSS NAV DATA", "R: GLONASS",
                "RINEX VERSION / TYPE");
    }
    fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->prog, opt->runby, date, "PGM / RUN BY / DATE");
...
extern int outrnxhnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    if (opt->rnxver <= 2.99)
    { /* ver.2 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver, "H: GEO NAV MSG DATA", "",
                "RINEX VERSION / TYPE");
    }
    else
    { /* ver.3 */
        fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver, "N: GNSS NAV DATA", "S: SBAS Payload",
                "RINEX VERSION / TYPE");
    }
    fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", opt->prog, opt->runby, date, "PGM / RUN BY / DATE");
...
extern int outrnxlnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver, "N: GNSS NAV DATA", "E: Galileo",
            "RINEX VERSION / TYPE");
...
extern int outrnxqnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver, "N: GNSS NAV DATA", "J: QZSS",
            "RINEX VERSION / TYPE");
...
extern int outrnxcnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver, "N: GNSS NAV DATA", "C: BeiDou",
            "RINEX VERSION / TYPE");
...
extern int outrnxinavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    fprintf(fp, "%9.2f           %-20s%-20s%-20s\n", opt->rnxver, "N: GNSS NAV DATA", "I: IRNSS",
            "RINEX VERSION / TYPE");
