...
static void geoparam(const double *pos, const double *vn, double *wen_n, double *wie_n, double *g)
{
...
/* update ins states---------------------------------------------------------*/
static void update(insstate_t *ins, const double *qbn, const double *vn, const double *Cen, const double h)
{
    double Cbn[9], rn[3];
    quat2dcmx(qbn, Cbn);
...
extern int updateinsbn(const insopt_t *insopt, insstate_t *ins, const imud_t *data)
{
    ...
    rot2dcm(w, Cn);
    ned2xyz(rn, Cne);
    matmul("NT", 3, 3, 3, 1.0, Cn, Cne, 0.0, Cen);
    h = rn[2] + vmid[2] * dt;
