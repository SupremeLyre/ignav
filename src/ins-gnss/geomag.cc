...
static int interpsh(const double date, double dte1, int nmax1, double dte2, int nmax2, const int gh,
                    const double gh1[MAXCOEFF], const double gh2[MAXCOEFF], double gha[MAXCOEFF], double ghb[MAXCOEFF]);
static int extrapsh(double date, double dte1, int nmax1, int nmax2, int gh, const double gh1[MAXCOEFF],
                    const double gh2[MAXCOEFF], double gha[MAXCOEFF], double ghb[MAXCOEFF]);
static int shval3(const CoordinateSystem coordSys, double flat, double flon, const double elev, const int nmax,
                  const int gh, int iext, double ext1, double ext2, double ext3, const double gha[MAXCOEFF],
                  const double ghb[MAXCOEFF], double *const x, double *const y, double *const z);
static void dihf(const double x, const double y, const double z, double *const d, double *const i, double *const h,
                 double *const f);
#ifndef TARGET_EMBEDDED
static int getshc(FILE *stream, int iflag, long int strec, int nmax_of_gh, const int gh, double gh1[MAXCOEFF],
                  double gh2[MAXCOEFF]);
#endif
 ...
 */
extern int get_field_components(BField *const bfield, BFieldModel const *const model, double alt, const Units altUnits,
                                const CoordinateSystem coordSys, const double latitude, const double longitude,
                                const double sdate)
{
    ...
    {
        nmax = extrapsh(sdate, model->epoch[modelI], model->max1[modelI], model->max2[modelI], 3, model->gh1[modelI],
                        model->gh2[modelI], gha, ghb);
    ...
    shval3(coordSys, latitude, longitude, alt, nmax, 3, IEXT, EXT_COEFF1, EXT_COEFF2, EXT_COEFF3, gha, ghb,
           &(bfield->x), &(bfield->y), &(bfield->z));
    dihf(bfield->x, bfield->y, bfield->z, &(bfield->d), &(bfield->i), &(bfield->h), &(bfield->f));
    shval3(coordSys, latitude, longitude, alt, nmax, 4, IEXT, EXT_COEFF1, EXT_COEFF2, EXT_COEFF3, gha, ghb, &xtemp,
           &ytemp, &ztemp);
    dihf(xtemp, ytemp, ztemp, &dtemp, &itemp, &htemp, &ftemp);

    bfield->ddot = ((dtemp - bfield->d) * RAD2DEG);
    if (bfield->ddot > 180.0)
        bfield->ddot -= 360.0;
    if (bfield->ddot <= -180.0)
        bfield->ddot += 360.0;
    bfield->ddot *= 60.0;
...
extern int read_model(BFieldModel *const model, const char mdfile[])
{
    ...
    while (fgets(inbuff, MAXREAD, stream))
    {
...
#endif
        {
            ...
            sscanf(inbuff, "%s%lg%d%d%d%lg%lg%lg%lg", model->name[modelI], &(model->epoch[modelI]),
                   &(model->max1[modelI]), &(model->max2[modelI]), &(model->max3[modelI]), &(model->yrmin[modelI]),
                   &(model->yrmax[modelI]), &(model->altmin[modelI]), &(model->altmax[modelI]));
    ...
    for (modelI = 1; modelI < model->nmodel; modelI++)
    {
        if (model->max2[modelI] == 0)
        {
            getshc(stream, 1, model->irec_pos[modelI], model->max1[modelI], 1, model->gh1[modelI], model->gh2[modelI]);
            getshc(stream, 1, model->irec_pos[modelI + 1], model->max1[modelI + 1], 2, model->gh1[modelI],
                   model->gh2[modelI]);
        }
        else
        {
            getshc(stream, 1, model->irec_pos[modelI], model->max1[modelI], 1, model->gh1[modelI], model->gh2[modelI]);
            getshc(stream, 0, model->irec_pos[modelI], model->max2[modelI], 2, model->gh1[modelI], model->gh2[modelI]);
        }
    }
    fclose(stream);
    return 1;
                  ...
                  double gh2[MAXCOEFF])
{
    ...
    for (nn = 1; nn <= nmax_of_gh; nn++)
    {
        for (mm = 0; mm <= nn; mm++)
        {
            ...
            {
                fgets(inbuff, MAXREAD, stream);
                sscanf(inbuff, "%d%d%lg%lg%lg%lg%s%d", &n, &m, &trash, &trash, &g, &hh, irat, &line_num);
            }
            if ((nn != n) || (mm != m))
            {
                fclose(stream);
