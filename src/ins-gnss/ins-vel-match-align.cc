...
static int cvmali(const double *pos, int np, double *dt, double *Cbn, int *ind, double *ve)
{
    ...
    for (k = 0, i = 0; i < np - 1; i++)
    {
        ...
        if (norm(vel + 3 * k, 3) < MINVELE || dt[i] > MININTERNAL)
            continue;
...
static int gt2ii(gtime_t time, int iis, int iin, const imu_t *imu)
{
    ...
    {
        if (fabs(timediff(imu->data[i].time, time)) < DTTOL)
            return i;
    }
    return 0;
}
 ...
 * return : 1: ok,0: fail
 * -------------------------------------------------------------------------*/
extern int cvmalign(const gsof_data_t *gsof, int igs, const imu_t *imu, int iis, const insopt_t *opt, int solq,
                    insstate_t *ins)
{
    ...
    for (i = igs; i < gsof->n; i++)
    {
        ...
        if (gsof->data[i].solq == solq)
        {
            ...
            if (norm(imu->data[iie].gyro, 3) > 90.0 * D2R)
                continue;
        ...
        {
            for (p = 0; p < k - 1; p++)
                dt[p] = timediff(time[p + 1], time[p]);
            if ((stat = cvmali(pos, k, dt, Cbn, &ind, ve)))
                break;
            k = 0;
        }
    }
    ...
    if (stat)
    {
        ...
        gapv2ipv(gsof->data[ind].pos, ve, ins->Cbe, opt->lever, &imu->data[iie], ins->re, ins->ve);
 ...
 * -------------------------------------------------------------------------*/
extern int easyvmali(const gsof_data_t *gsof, int igs, const imu_t *imu, int iis, const insopt_t *opt, int solq,
                     insstate_t *ins)
{
    int i, j, k, l;
    int in = (int)opt->hz * 2;
    ...
    for (k = 0, i = igs; i < gsof->n; i++)
    {
        ...
        if (k == 2)
        {
            if (fabs(dt = timediff(time[1], time[0])) > MININTERNAL)
            {
                k = 0;
                continue;
            }
            ...
            for (j = 0; j < 3; j++)
                ve[j] = (pos[j + 3] - pos[j]) / dt;

            if (fabs(norm(ve, 3)) < MINVELE)
            {
                k = 0;
                continue;
            }
            ...
            if (!(l = gt2ii(time[1], iis, in, imu)))
            {
                k = 0;
                continue;
            }

            gapv2ipv(pos, ve, ins->Cbe, opt->lever, &imu->data[l], ins->re, ins->ve);
 ...
 * --------------------------------------------------------------------------*/
extern int inputobs(int revs, const obs_t *obss, obsd_t *obs, const prcopt_t *popt, int *iobsu, int *iobsr, int *nuo,
                    int *nro)
{
    int i, nu, nr, n = 0;
 ...
 * return : 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int alisgpos(const obs_t *obs, const nav_t *nav, const imu_t *imu, const prcopt_t *opt, insstate_t *ins,
                    int *iobss, int *iimu)
{
    obsd_t data[MAXOBS];
    gtime_t ts = {0};
    ...
    for (np = 0, iobs = 0; (m = nextobsf(obs, &iobs, 1)) > 0; iobs += m)
    {
        ...
        if (!pntpos(data, j, nav, opt, sol + np, NULL, NULL, NULL, msg))
            continue;
        ...
        if (np == n)
        {
            ...
            if (norm(dt, np - 1) > SQRT(np - 1) || norm(dt, np - 1) == 0.0)
            {
                np = 0;
                continue;
            }
 ...
 * return : 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int alirtkpos(const obs_t *obs, const nav_t *nav, const imu_t *imu, const prcopt_t *opt, insstate_t *ins,
                     int *iobsu, int *iobsr, int *iimu)
{
    ...
    /* position mode */
    if (opt->insopt.alimethod == INSALIGN_DGPS)
        popt.mode = PMODE_DGPS;
    else if (opt->insopt.alimethod == INSALIGN_RTK)
    ...
    while ((nobs = inputobs(0, obs, data, &popt, &iu, &ir, &nu, &nr)) >= 0)
    {
        ...
        for (i = n = 0; i < nobs; i++)
        {
            if ((satsys(data[i].sat, NULL) & popt.navsys) && popt.exsats[data[i].sat - 1] != 1)
                data[n++] = data[i];
        }
        if (n <= 0)
            continue;
#if 1
        if (!screent(data[0].time, ts, ts, 1.0))
            continue;
#endif
        ...
        if (k == np)
        {
            ...
            if (!ant2inins(solp[k - 1].time, rr, vr, &popt.insopt, imu, ins, iimu))
            {
                k = 0;
                continue;
            }
 ...
 * return : 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int alippkpos(const obs_t *obs, const nav_t *nav, const imu_t *imu, const prcopt_t *opt, insstate_t *ins,
                     int *iobsu, int *iimu)
{
    trace(3, "alirtkpos:\n");
    return 1;
