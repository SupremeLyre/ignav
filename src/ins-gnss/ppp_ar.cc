...
static void average_LC(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav, const double *azel)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (rtk->ssat[sat - 1].slip[0] || rtk->ssat[sat - 1].slip[1] || rtk->ssat[sat - 1].slip[2] ||
            amb->n[0] == 0.0 || fabs(timediff(amb->epoch[0], obs[0].time)) > MIN_ARC_GAP)
        {
...
static int fix_amb_WL(rtk_t *rtk, const nav_t *nav, int sat1, int sat2, int *NW)
{
    ...
    *NW = ROUND(BW);
    ...
    return fabs(*NW - BW) <= rtk->opt.thresar[2] && conffunc(*NW, BW, sqrt(vW)) >= rtk->opt.thresar[1];
}
...
/* fixed solution ------------------------------------------------------------*/
static int fix_sol(rtk_t *rtk, const int *sat1, const int *sat2, const double *NC, int n)
{
    double *v, *H, *R;
    int i, j, k, info;
...
static int fix_amb_ROUND(rtk_t *rtk, int *sat1, int *sat2, const int *NW, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (fabs(N1 - B1) > rtk->opt.thresar[2] || conffunc(N1, B1, sqrt(v1)) < rtk->opt.thresar[1])
        {
            continue;
        }
...
extern int ppp_ar(rtk_t *rtk, const obsd_t *obs, int n, int *exc, const nav_t *nav, const double *azel, double *x,
                  double *P)
{
    double elmask;
    ...
    for (i = 0; i < n - 1; i++)
        for (j = i + 1; j < n; j++)
        {
            ...
            if (!rtk->ssat[obs[i].sat - 1].vsat[0] || !rtk->ssat[obs[j].sat - 1].vsat[0] || azel[1 + i * 2] < elmask ||
                azel[1 + j * 2] < elmask)
                continue;
#if 0
            /* test already fixed */
