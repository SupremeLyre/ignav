...
static void meascov(const pose_meas_t *pose, const sol_t *sol, double *cov, double *Cvn)
{
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            W[i + i * 6] = cov_vn[i + j * 3];
    for (i = 3; i < 6; i++)
    {
        for (j = 3; j < 6; j++)
            W[i + j * 6] = cov_bn[(i - 3) + (j - 3) * 3];
    }
    for (i = 0; i < 9; i++)
    {
        I[i] = T[i];
...
/* ekf filter matrix---------------------------------------------------------*/
static int build_ekf(const insopt_t *opt, const sol_t *sol, const pose_meas_t *pose, const double *Cvb0, double *v,
                     double *H, double *R)
{
...
static void addQ(double *P, int nx)
{
    int i;
    for (i = 0; i < nx; i++)
        P[i + i * nx] += SQR(5E-5 * D2R);
}
/* build ekf filter matrix---------------------------------------------------*/
static int calibfilt(const insopt_t *opt, const solbuf_t *solbuf, posebuf_t *posebuf, gtime_t ts, gtime_t te, double *C)
{
    ...
    for (i = 0; i < solbuf->n; i++)
    {
        ...
        addQ(P, 3);
#endif
        if (!screent(solbuf->data[i].time, ts, te, 0.0))
            continue;
        ...
        /* measurement matrix */
        if (!(nv = build_ekf(opt, &solbuf->data[i], &posebuf->data[j], Cvb, v, H, R)))
        {
            continue;
        }
        /* ekf filter */
...
static int post_proc(const insopt_t *opt, const solbuf_t *solbuf, posebuf_t *posebuf, gtime_t ts, gtime_t te,
 ...
 * --------------------------------------------------------------------------*/
static int initmisali(const insopt_t *opt, const solbuf_t *solbuf, posebuf_t *posebuf, gtime_t ts, gtime_t te,
                      double *Cvb)
{
 ...
 * return: 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int calibdualant(const insopt_t *opt, const solbuf_t *solbuf, posebuf_t *posebuf, gtime_t ts, gtime_t te,
                        double *Cvb, double *ave, double *std)
{
    trace(3, "calibdualant:\n");

    if (solbuf->n <= 0 || posebuf->n <= 0)
        return 0;
 ...
 * return: 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int calibdualantx(const insopt_t *opt, const solbuf_t *solbuf, posebuf_t *posebuf, gtime_t ts, gtime_t te,
