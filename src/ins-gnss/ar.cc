...
/* average LC ----------------------------------------------------------------*/
static void average_LC(rtk_t *rtk, const obsd_t *obs, const int *sat, const int *iu, const int *ir, int ns,
                       const nav_t *nav, const double *azel)
{
    ...
    for (i = 0; i < ns; i++)
    {
        ...
        if (rtk->ssat[sat[i] - 1].slip[0] || rtk->ssat[sat[i] - 1].slip[1] || rtk->ssat[sat[i] - 1].slip[2] ||
            amb->n[0] == 0 || fabs(timediff(amb->epoch[0], obs[0].time)) > MIN_ARC_GAP)
        {
                       ...
                       const nav_t *nav, const double *azel)
{
    ...
    if (ns <= 0 || rtk->opt.ionoopt != IONOOPT_IFLC || rtk->opt.nf < 2)
        return 0;
    ...
    for (j = 0; j < ns; j++)
    {
        ...
        fix = fabs(NW - BW) <= FIX_THRES * 1.5 && sqrt(vW) <= FIX_THRES && fabs(N1 - B1) <= FIX_THRES * 1.5 &&
              sqrt(v1) <= FIX_THRES;

        trace(3, "%s sat=%2d-%2d B=%13.3f %13.3f sig=%7.3f %7.3f %s\n", time_str(obs[0].time, 0), sat[i], sat[j], BW,
              B1, sqrt(vW), sqrt(v1), fix ? "FIX" : "---");

        if (!fix || ++ambj->fixcnt < MIN_FIX_CNT)
            continue;
                       ...
                       const nav_t *nav, const double *azel)
{
    ...
    if (ns <= 0 || rtk->opt.ionoopt != IONOOPT_IFLC || rtk->opt.nf < 3)
        return 0;
    ...
    for (j = 0; j < ns; j++)
    {
        ...
        if (!ambi->n[1] || !ambj->n[1] || !ambi->n[2] || !ambj->n[2])
            continue;
        ...
        fix = fabs(NE - BE) <= FIX_THRES * 1.5 && sqrt(vE) <= FIX_THRES && fabs(NF - BF) <= FIX_THRES * 1.5 &&
              sqrt(vF) <= FIX_THRES && fabs(N1 - B1) <= FIX_THRES * 1.5 && sqrt(v1) <= FIX_THRES;

        trace(1, "%s sat=%2d-%2d B=%13.3f %13.3f %13.3f sig=%7.3f %7.3f %7.3f %s\n", time_str(obs[0].time, 0), sat[i],
              sat[j], BE, BF, B1, sqrt(vE), sqrt(vF), sqrt(v1), fix ? "FIX" : "---");
