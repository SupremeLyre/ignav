...
static int chk_static(const double *dT, int status, gtime_t time)
{
    ...
    for (i = 0; i < NUMPOSE; i++)
    {
        if (!T[i].gtime.time || T[i].status <= 0)
            return 0;
    }
    for (i = 0; i < NUMPOSE; i++)
    {
 ...
 * --------------------------------------------------------------------------*/
extern int zvu_cam(const insopt_t *opt, const double *dT, int status, const imud_t *imu, gtime_t tc, insstate_t *ins)
{
 ...
 * return : 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int initcamgps(gtime_t time, const double *r, const insopt_t *opt, insstate_t *ins)
{
    ...
    if (fabs(timediff(ts[1], ts[0])) < 1E-5)
        return 0;
    for (i = 0; i < 2; i++)
    {
        if (norm(rr + 3 * i, 3) <= 0.0)
            return 0;
    }
    for (i = 0; i < 3; i++)
    {
 ...
 * return: 1 (ok) or 0 (fail)
 * --------------------------------------------------------------------------*/
extern int updatecam(insstate_t *ins, const insopt_t *opt, const double *dT, gtime_t time)
{
    ...
    {
        if (dt > 0.0)
            ins->vo.vc[i] = (ins->vo.rc[i] - rp[i]) / dt;
        else
        {
            ins->vo.vc[i] = 0.0;
        }
