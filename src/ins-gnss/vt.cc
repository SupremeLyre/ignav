...
static int seq_telnet(vt_t *vt)
{
    ...
    else if (vt->esc[1] == C_WONT || vt->esc[1] == C_DONT)
    { /* option negotiation */
        if (vt->nesc < 3)
            return 1;
    ...
    else if (vt->esc[1] == C_BRK || vt->esc[1] == C_IP)
    { /* break or interrupt */
        vt->brk = 1;
    }
    else if (vt->esc[1] == C_EC)
    { /* erase character */
        del_cur(vt);
    }
    else if (vt->esc[1] == C_EL)
    { /* erase line */
        clear_buff(vt);
    }
    vt->nesc = 0;
...
extern int vt_getc(vt_t *vt, char *c)
{
    ...
    *c = '\0';
    ...
    if ((vt->type && *c == C_IAC) || *c == C_ESC)
    { /* escape or telnet */
        vt->esc[0] = *c;
        *c = '\0';
        vt->nesc = 1;
    }
    else if (vt->nesc > 0 && vt->esc[0] == C_IAC)
    { /* telnet sequence */
        vt->esc[vt->nesc++] = *c;
        *c = '\0';
        if (!seq_telnet(vt))
    ...
    else if (vt->nesc > 0 && vt->esc[0] == C_ESC)
    { /* escape sequence */
        vt->esc[vt->nesc++] = *c;
        *c = '\0';
        if (!seq_esc(vt))
