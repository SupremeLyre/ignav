/*------------------------------------------------------------------------------
 * ins-sim.c : ins simulation functions
 *
 * reference :
 ...
 * --------------------------------------------------------------------------*/
extern int sim_imu_static(const double *rpy, const double *pos, const double ts, const double T, const imu_err_t *err,
                          imu_t *data)
{
    int i, j, n;
    ...
    for (i = 0; i < n; i++)
    {
        for (j = 0; j < 3; j++)
        {
            ...
            data->data[i].gyro[j] = wbie[j] + err->bg[j] + err->wbg[j] / sqrt(ts) * getgaussian(1.0);
            data->data[i].accl[j] = -gn[j] + err->ba[j] + err->wba[j] / sqrt(ts) * getgaussian(1.0);
        }
    }
    data->coor = IMUCOOR_FRD;
    data->decfmt = IMUDECFMT_RATE;
    return n;
}