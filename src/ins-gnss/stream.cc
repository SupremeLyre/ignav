...
{
    const int br[] = {300, 600, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400};
    serial_t *serial;
    ...
    char dcb[64] = "";
#else
    const speed_t bs[] = {B300, B600, B1200, B2400, B4800, B9600, B19200, B38400, B57600, B115200, B230400};
...
static tcpsvr_t *opentcpsvr(const char *path, char *msg)
{
    tcpsvr_t *tcpsvr = NULL, tcpsvr0 = {{0}};
    char port[256] = "";
...
static ntripc_t *openntripc(const char *path, int type, char *msg)
{
    ...
    for (i = 0; i < MAXCLI; i++)
    {
        ...
        ntripc->con[i].nb = 0;
        for (j = 0; j < NTRIP_MAXRSP; j++)
            ntripc->con[i].buff[j] = 0;
    }
...
static int test_mntpnt(ntripc_t *ntripc, const char *mntpnt)
{
    ...
    {
        if (sscanf(p, "STR;%255[^;]", str) && !strcmp(str, mntpnt))
            break;
    }
    unlock(&ntripc->lock_srctbl);
...
#endif
{
    ...
    char cmd[2048], env[1024] = "", opt[1024], *proxyopt = "", proto[1024];
    int ret;
    ...
    if (*proxyaddr)
    {
        strcpy(proto, ftp->proto ? "http" : "ftp");
    ...
    if (ftp->proto == 0)
    { /* ftp */
        sprintf(opt, "--ftp-user=%s --ftp-password=%s --glob=off --passive-ftp %s-t 1 -T %d -O \"%s\"", ftp->user,
                ftp->passwd, proxyopt, FTP_TIMEOUT, local);
