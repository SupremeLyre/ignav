...
extern void distortradtan(const cam_t *cam, const double *in, double *out, double *J)
{
    ...
    {
        J[0] =
            1.0 + rad + 2.0 * cam->k1 * mx2 + cam->k2 * r2 * 4.0 * mx2 + 2.0 * cam->p1 * in[1] + 6.0 * cam->p2 * in[0];
        J[1] = 2.0 * cam->k1 * mxy + 4.0 * cam->k2 * r2 * in[0] * in[1] + 2.0 * cam->p1 * in[0] + 2.0 * cam->p2 * in[1];
        J[2] = J[1];
        J[3] =
            1.0 + rad + 2.0 * cam->k1 * my2 + cam->k2 * r2 * 4.0 * my2 + 6.0 * cam->p1 * in[1] + 2.0 * cam->p2 * in[0];
    }
}
 ...
 * --------------------------------------------------------------------------*/
extern int undistortradtan(const cam_t *cam, const double *in, double *out, double *J)
{
    ...
    matcpy(ybar, in, 1, 2);
    for (i = 0; i < 5; i++)
    { /* iterations for undistorted */

        matcpy(ytmp, ybar, 1, 3);
        distortradtan(cam, ytmp, ytmp, F);
 ...
 * args  :  cam_t *cam  I  camera model
 *          double *P   I  3D point in camera-frame
 *          double *p   O  2D point (u,v) in image-frame
 *          double *J   O  jacobians of p(u,v) wrt P(x,y,z)
 * return: none
 * --------------------------------------------------------------------------*/
extern void prj3d2img(const cam_t *cam, const double *P, feature_t *p, double *J)
{
    double J0[4] = {0}, J1[4] = {0}, J2[6] = {0};
    double xn[2], xd[3] = {1.0}, xp[3];

    trace(3, "prj3d2img:\n");

    xn[0] = P[0] / P[2];
    xn[1] = P[1] / P[2];
    distortradtan(cam, xn, xd, J1);

    matmul("NN", 3, 1, 3, 1.0, cam->K, xd, 0.0, xp);
    ...
    if (J)
    {
        ...
        J2[0] = 1.0 / P[2];
        J2[3] = 1.0 / P[2];
        matmul33("NNN", J0, J1, J2, 2, 2, 2, 3, J);
    }
}